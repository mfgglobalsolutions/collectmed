...
<!--- <strong>filename</strong> (2004FeeScheduleRegionC.txt)     --->
<!---<br>                                                        --->
...
<!-------------------------------------------------------------------------------------->
	...
	<cfparam name="attributes.filepath" default="C:\EOBManager\eob_1002_Codes_Import\feeCodesFees\2004">
	<cfset filepath = attributes.filepath>

	<cfparam name="attributes.filename" default="DMEFeeSchedule_2004_PIPE.txt">
	...
	<cfloop list="#trim(fileContent)#" delimiters="#chr(13)##chr(10)#" index="i">
		...
		<!---<cfif NOT ISNumeric(trim(getCategoryCodeID.StandardListItemID))>--->		
			<cfoutput>
				...
				INSERT INTO pa_master.EOB_MEDICARE_2004_PROCEDURECodeFeeSchedule(HCPCRecordID, CategoryID, MOD1, Fee_AL_16, Fee_AR_19, Fee_CO_21, Fee_FL_25, Fee_GA_26, Fee_KY_33, Fee_LA_34, Fee_MS_40, Fee_NM_47, Fee_NC_49, Fee_OK_52, Fee_SC_56, Fee_TN_58, Fee_TX_59, Fee_PR_229, Fee_VI_238)
				VALUES(#trim(HCPCRecordID)#,#trim(getCategoryCodeID.StandardListItemID)#, #trim(MOD1)#, #trim(AL)#, #trim(AR)#, #trim(CO)#, #trim(FL)#, #trim(GA)#, #trim(KY)#, #trim(LA)#, #trim(MS)#, #trim(NM)#, #trim(NC)#, #trim(OK)#, #trim(SC)#, #trim(TN)#, #trim(TX)#, #trim(PR)#, #trim(VI)#)
				<br><br>
			</cfoutput>		
		<!---</cfif>		--->
