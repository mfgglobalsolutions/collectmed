...
<!--- NAME:                                                      --->
<!--- @@NAME@@                                                   --->
<!---   ei_1002_InsertUpdateAcuteCareDiagnosisCodes.cfm          --->
<!--- @@NAME@@                                                   --->
<!------------------------------------------------------------------>
<!--- CATEGORY:                                                  --->
<!--- @@CATEGORY@@                                               --->
...
<!--- eobmanager\eob_1002_Codes_Import\DIAGDESC.REF              --->
<!--- Downloaded from tmhp                                       --->
<!--- This tag will read the file above and place the DIAGDESC   --->
...
<!--- EXAMPLE CALL:                                              --->
<!--- @@EXAMPLECALL@@                                            --->
<!---  &lt;cf_ei_1002_InsertUpdateAcuteCareDiagnosisCodes<br>    --->
<!---  &nbsp;&nbsp;testmode="no"                                 --->
<!--- @@EXAMPLECALL@@                                            --->
<!---                                                            --->
<!------------------------------------------------------------------>
...
<!--- <strong>filename</strong> (DIAGDESC.REF)<br>               --->
...
<!------------------------------------------------------------------>
	...
	<cfparam name="attributes.filename" default="DIAGDESC.REF">
...
<!------------------------------------------------------------------>
	<cfloop list="#trim(fileContent)#" delimiters="#chr(13)##chr(10)#" index="i">
		...
		<cfset code =  trim(MID(i, 1, 5))>
		<cfset description =  trim(MID(i, 6, len(i) - 5))>
		
		<cfquery name="getCode" datasource="#trim(request.datasource)#">
			SELECT recordID 
			FROM ei_2_DIAGNOSISCode  
		...
		<cfelse>
			...
			<cfif NOT testmode>
				...
				<cfquery name="insertCode" datasource="#trim(request.datasource)#">
					INSERT INTO ei_2_DIAGNOSISCode  (code, description)
