...
<!--- <strong>filename</strong> (2009FeeScheduleRegionC.txt)     --->
<!---<br>                                                        --->
...
<!-------------------------------------------------------------------------------------->
	...
	<cfparam name="attributes.filepath" default="C:\EOBManager\eob_1002_Codes_Import\feeCodesFees\2009">
	<cfset filepath = attributes.filepath>

	<cfparam name="attributes.filename" default="DMEFeeSchedule_2009_A_PIPE.txt">
	<cfset filename = attributes.filename>
	
	<cfparam name="attributes.modifiers" default="AU,AV,AW,BA,KC,KF,KM,KN,NU,RR,UE,KL,KE">
...
<!---HCPCS|Mods|Category|AL|AR|CO|FL|GA|LA|MS|NC|NM|OK|SC|TN|TX|PR|VI--->
	...
	<cfloop list="#trim(fileContent)#" delimiters="#chr(13)##chr(10)#" index="i">
		...
		<cfif testMode><cfoutput>[#count#]</cfoutput></cfif>			
		...
		<cfelse>
			<cfoutput>
				INSERT INTO EOB_MEDICARE_2009_A_PROCEDURECodeFeeSchedule(HCPCRecordID, CategoryID, MOD1, MOD2, MOD3, MOD4, Ceiling, Floor, Fee_AL_16, Fee_AK_17, Fee_AZ_18, Fee_AR_19, Fee_CA_20, Fee_CO_21, Fee_CT_22, Fee_DE_23, Fee_DC_24, Fee_FL_25, Fee_GA_26, Fee_HI_27, Fee_ID_28, Fee_IL_29, Fee_IN_30, Fee_IA_31, Fee_KS_32, Fee_KY_33, Fee_LA_34, Fee_ME_35, Fee_MD_36, Fee_MA_37, Fee_MI_38, Fee_MN_39, Fee_MS_40, Fee_MO_41, Fee_MT_42, Fee_NE_43, Fee_NV_44, Fee_NE_45, Fee_NJ_46, Fee_NM_47, Fee_NY_48, Fee_NC_49, Fee_ND_50, Fee_OH_51, Fee_OK_52, Fee_OR_53, Fee_PA_54, Fee_RI_55, Fee_SC_56, Fee_SD_57, Fee_TN_58, Fee_TX_59, Fee_UT_60, Fee_VT_61, Fee_VA_62, Fee_WA_63, Fee_WV_64, Fee_WI_65, Fee_WY_66, Fee_PR_229, Fee_VI_238, Description)<br>
				VALUES(#trim(getCode.RecordID)#,#trim(getCategoryCodeID.StandardListItemID)#, #trim(MOD1)#, #trim(MOD2)#, NULL, NULL, #trim(Ceiling)#, #trim(Floor)#, #trim(AL)#, #trim(AK)#, #trim(AZ)#, #trim(AR)#, #trim(CA)#, #trim(CO)#, #trim(CT)#, #trim(DE)#, #trim(DC)#, #trim(FL)#, #trim(GA)#, #trim(HI)#, #trim(ID)#, #trim(IL)#, #trim(IN)#, #trim(IA)#, #trim(KS)#, #trim(KY)#, #trim(LA)#, #trim(ME)#, #trim(MD)#, #trim(MA)#, #trim(MI)#, #trim(MN)#, #trim(MS)#, #trim(MO)#, #trim(MT)#, #trim(NE)#, #trim(NV)#, #trim(NE)#, #trim(NJ)#, #trim(NM)#, #trim(NY)#, #trim(NC)#, #trim(ND)#, #trim(OH)#, #trim(OK)#, #trim(ORe)#, #trim(PA)#, #trim(RI)#, #trim(SC)#, #trim(SD)#, #trim(TN)#, #trim(TX)#, #trim(UT)#, #trim(VT)#, #trim(VA)#, #trim(WA)#, #trim(WV)#, #trim(WI)#, #trim(WY)#, #trim(PR)#, #trim(VI)#, '#trim(Description)#')
				<br><br>
			</cfoutput>		
		</cfif>				
...
ALTER TABLE [dbo].[EOB_MEDICARE_2009_A_PROCEDURECodeFeeSchedule] ADD 
	...
	CONSTRAINT [DF_EOB_MEDICARE_2009_A_PROCEDURECodeFeeSchedule_DateCreated] DEFAULT (now()) FOR [DateCreated]
GO
