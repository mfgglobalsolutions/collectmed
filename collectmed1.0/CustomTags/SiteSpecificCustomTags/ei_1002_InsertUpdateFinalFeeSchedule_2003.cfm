...
<!--- <strong>filename</strong> (2003FeeScheduleRegionC.txt)     --->
<!---<br>                                                        --->
...
<!-------------------------------------------------------------------------------------->
	...
	<cfparam name="attributes.filepath" default="C:\EOBManager\eob_1002_Codes_Import\feeCodesFees\2003">
	<cfset filepath = attributes.filepath>

	<cfparam name="attributes.filename" default="DMEFeeSchedule_2003_PIPE.txt">
...
<!------------------------------------------------------------------>
	...
	<cfloop list="#trim(fileContent)#" delimiters="#chr(13)##chr(10)#" index="i">
		...
		<cfset KY = ListGetAt(i,9,"|")>
		<cfset LA = ListGetAt(i,10,"|")>
		<cfset MS = ListGetAt(i,11,"|")>
		<cfset NC = ListGetAt(i,12,"|")>
		<cfset NM = ListGetAt(i,13,"|")>
		<cfset OK = ListGetAt(i,14,"|")>
		<cfset SC = ListGetAt(i,15,"|")>
		<cfset TN = ListGetAt(i,16,"|")>
		<cfset TX = ListGetAt(i,17,"|")>
		<cfset PR = ListGetAt(i,18,"|")>
		<cfset VI = ListGetAt(i,19,"|")>		
		...
		<!---<cfif NOT ISNumeric(trim(getCategoryCodeID.StandardListItemID))>--->		
			<cfoutput>
				[#count#][#trim(HCPCS)#][#trim(Category)#]<br>                                                                                                                                
				INSERT INTO pa_master.EOB_MEDICARE_2003_PROCEDURECodeFeeSchedule(HCPCRecordID, CategoryID, MOD1, Fee_AL_16, Fee_AR_19, Fee_CO_21, Fee_FL_25, Fee_GA_26, Fee_KY_33, Fee_LA_34, Fee_MS_40, Fee_NM_47, Fee_NC_49, Fee_OK_52, Fee_SC_56, Fee_TN_58, Fee_TX_59, Fee_PR_229, Fee_VI_238)
				VALUES(#trim(HCPCRecordID)#,<cfif Category EQ "NULL">NULL<cfelse>#trim(getCategoryCodeID.StandardListItemID)#</cfif>, #trim(MOD1)#, #trim(AL)#, #trim(AR)#, #trim(CO)#, #trim(FL)#, #trim(GA)#, #trim(KY)#, #trim(LA)#, #trim(MS)#, #trim(NM)#, #trim(NC)#, #trim(OK)#, #trim(SC)#, #trim(TN)#, #trim(TX)#, #trim(PR)#, #trim(VI)#)
				<br><br>
			</cfoutput>		
		<!---</cfif>		--->
