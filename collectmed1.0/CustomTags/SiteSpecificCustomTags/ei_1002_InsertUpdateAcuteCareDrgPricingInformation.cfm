...
<!---
NOT YET FINISHED DID NOT NEED FOR FIRST RELEASE OF POLY-ALLIENCE
--->
...
<!--- NAME:                                                      --->
<!--- @@NAME@@                                                   --->
<!---   ei_1002_InsertUpdateAcuteCareDrgPricingInformation.cfm   --->
<!--- @@NAME@@                                                   --->
<!------------------------------------------------------------------>
<!--- CATEGORY:                                                  --->
<!--- @@CATEGORY@@                                               --->
...
<!--- eobmanager\eob_1002_Codes_Import\DRGDTL.REF                --->
<!--- Downloaded from tmhp                                       --->
<!--- DRG: Diagnosis Related Group                               --->
<!--- This tag will read the file above and place the DRG        --->
...
<!--- @@EXAMPLECALL@@                                            --->
<!---  &lt;                                                      --->
<!--- cf_ei_1002_InsertUpdateAcuteCareDrgPricingInformation<br>  --->
<!---  &nbsp;&nbsp;testmode="no"                                 --->
<!--- @@EXAMPLECALL@@                                            --->
<!---                                                            --->
<!------------------------------------------------------------------>
...
<!--- <strong>filename</strong> (DRGDTL.REF)<br>                 --->
...
<!------------------------------------------------------------------>
	...
	<cfparam name="attributes.filename" default="DRGDTL.REF">
...
<!------------------------------------------------------------------>
	<cfloop list="#trim(fileContent)#" delimiters="#chr(13)##chr(10)#" index="i">
		...
		<cfset code =  MID(i, 1, 5)>
		<cfset description =  MID(i, 6, len(i) - 5)>
		
		<cfquery name="getCode" datasource="#trim(request.datasource)#">
			SELECT recordID 
			FROM ei_2_DRGPRICINGINFORMATION  
		...
		<cfif getCode.RecordCount GTE 1>
			...
			<cfif NOT testmode>
				...
				<cfquery name="update" datasource="#trim(request.datasource)#">
					UPDATE ei_2_DRGPRICINGINFORMATION  
		...
		<cfelse>
			...
			<cfif NOT testmode>
				...
				<cfquery name="insertCode" datasource="#trim(request.datasource)#">
					INSERT INTO ei_2_DRGPRICINGINFORMATION  (code, description)
