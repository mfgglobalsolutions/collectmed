...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the FileProcess table in the dB.                 --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_FileProcessTable                                                 --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="FileProcessIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.FileProcessID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "FILEPROCESSID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "94", 3)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for FileProcessID.                                          --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getFileProcessID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.FileProcessID>
	</cffunction>
	 
	<cffunction name="setFileProcessID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="FileProcessID" required="Yes" type="String">
		
		<cfif FileProcessID NEQ "NULL">	
			
			<cfif arguments.FileProcessID EQ "@@" OR arguments.FileProcessID EQ "">
				
				<cfset arguments.FileProcessID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(FileProcessID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>FileProcessID</strong> (#FileProcessID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.FileProcessID = arguments.FileProcessID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="FileProcessIO" output="No">
		
		<cfargument name="FileProcessID" required="yes" type="numeric">		
		<cfset var qGetFileProcess = "">
		
		<cfset preInit(FileProcessID)>
		...
		<cfscript>
			IF(qGetFileProcess.FileProcessID EQ "" OR qGetFileProcess.FileProcessID EQ "NULL") 
				variables.instance.FileProcessID = "NULL"; 
			ELSE 
				variables.instance.FileProcessID = qGetFileProcess.FileProcessID;
				
			IF(qGetFileProcess.FileID EQ "" OR qGetFileProcess.FileID EQ "NULL") 
				variables.instance.FileID = "NULL"; 
			ELSE 
				variables.instance.FileID = qGetFileProcess.FileID;
				
			IF(qGetFileProcess.StatusID EQ "" OR qGetFileProcess.StatusID EQ "NULL") 
				variables.instance.StatusID = "NULL"; 
			ELSE 
				variables.instance.StatusID = qGetFileProcess.StatusID;
							
				variables.instance.Note = qGetFileProcess.Note;
							
				variables.instance.Active = qGetFileProcess.Active;
				
			IF(qGetFileProcess.InactiveCode EQ "" OR qGetFileProcess.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetFileProcess.InactiveCode;
							
				variables.instance.DateCreated = qGetFileProcess.DateCreated;
							
				variables.instance.DateModified = qGetFileProcess.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(FileProcessID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitFileProcess = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.FileProcessID) AND variables.instance.FileProcessID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitFileProcess" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitFileProcess" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.FileProcessID = qInsertCommitFileProcess.FileProcessID>
				
				<cfreturn qInsertCommitFileProcess.FileProcessID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "FileProcessID,FileID,StatusID,Note,Active,InactiveCode,DateCreated,DateModified">		
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="FileID,StatusID,Note,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getFileProcessQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getFileProcessQuery">						
		<cfargument name="Fields" required="No" type="String" default="FileProcessID,FileID,StatusID,Note,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="FileProcessID" required="No" type="String" default="">
		<cfargument name="FileProcessID_IN" required="No" type="String" default="">
		<cfargument name="FileProcessID_LIKE" required="No" type="String" default="">
		<cfargument name="FileID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getFileProcessQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif FileProcessID_LIKE NEQ "">AND FileProcessID LIKE '%#trim(FileProcessID_LIKE)#%'</cfif>
