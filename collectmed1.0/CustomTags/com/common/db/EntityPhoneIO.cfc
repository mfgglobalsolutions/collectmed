...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EntityPhone table in the dB.                 --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EntityPhoneTable                                                 --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EntityPhoneIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PHONEID", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ISDEFAULT", 4)>
	...
	<cffunction name="setPhoneID" access="public" returntype="boolean" output="No">
		...
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "PhoneID")>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EntityPhoneIO" output="No">
		...
		<cfscript>
			IF(qGetEntityPhone.recordID EQ "" OR qGetEntityPhone.recordID EQ "NULL") 
				variables.instance.recordID = "NULL"; 
			ELSE 
				variables.instance.recordID = qGetEntityPhone.recordID;
				
			IF(qGetEntityPhone.EntityID EQ "" OR qGetEntityPhone.EntityID EQ "NULL") 
				variables.instance.EntityID = "NULL"; 
			ELSE 
				variables.instance.EntityID = qGetEntityPhone.EntityID;
				
			IF(qGetEntityPhone.PhoneID EQ "" OR qGetEntityPhone.PhoneID EQ "NULL") 
				variables.instance.PhoneID = "NULL"; 
			ELSE 
				variables.instance.PhoneID = qGetEntityPhone.PhoneID;
							
				variables.instance.IsDefault = qGetEntityPhone.IsDefault;
							
				variables.instance.Active = qGetEntityPhone.Active;
				
			IF(qGetEntityPhone.InactiveCode EQ "" OR qGetEntityPhone.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEntityPhone.InactiveCode;
							
				variables.instance.DateCreated = qGetEntityPhone.DateCreated;
							
				variables.instance.DateModified = qGetEntityPhone.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
		...
		<cfif IsNumeric(variables.instance.recordID) AND variables.instance.recordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEntityPhone" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEntityPhone" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.recordID = qInsertCommitEntityPhone.recordID>
				
				<cfreturn qInsertCommitEntityPhone.recordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "recordID,EntityID,PhoneID,IsDefault,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "IsDefault,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EntityPhone  SET #columnsToUpdate# WHERE recordID = #trim(variables.instance.recordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="EntityID,PhoneID,IsDefault,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEntityPhoneQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEntityPhoneQuery">						
		<cfargument name="Fields" required="No" type="String" default="recordID,EntityID,PhoneID,IsDefault,Active,InactiveCode,DateCreated,DateModified">	
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "recordID,recordID_IN,recordID_LIKE,EntityID,EntityID_IN,EntityID_LIKE,PhoneID,PhoneID_IN,PhoneID_LIKE,IsDefault,IsDefault_IN,IsDefault_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
