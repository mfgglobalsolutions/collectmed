...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Role table in the dB.                        --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ROLEID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ROLENAME", 2)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for RoleName.                                               --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getRoleName" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.RoleName>
	</cffunction>
	 
	<cffunction name="setRoleName" access="public" returntype="boolean" output="No">
 		
		<cfargument name="RoleName" required="Yes" type="String">
		
		<cfif RoleName NEQ "NULL">	
			
			<cfif arguments.RoleName EQ "@@" OR arguments.RoleName EQ "">
				
				<cfset arguments.RoleName = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(RoleName)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>RoleName</strong> (#RoleName#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(RoleName) GT 50>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>RoleName</strong> cannot be greater than 50 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.RoleName = arguments.RoleName>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "RoleName")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="RoleName" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="RoleIO" output="No">
		
		<cfargument name="RoleID" required="yes" type="numeric">		
		<cfset var qGetRole = "">
		
		<cfset preInit(RoleID)>
		...
		<cfscript>
			IF(qGetRole.RoleID EQ "" OR qGetRole.RoleID EQ "NULL") 
				variables.instance.RoleID = "NULL"; 
			ELSE 
				variables.instance.RoleID = qGetRole.RoleID;
							
				variables.instance.RoleName = qGetRole.RoleName;
				
			IF(qGetRole.SiteID EQ "" OR qGetRole.SiteID EQ "NULL") 
				variables.instance.SiteID = "NULL"; 
			ELSE 
				variables.instance.SiteID = qGetRole.SiteID;
							
				variables.instance.Active = qGetRole.Active;
				
			IF(qGetRole.InactiveCode EQ "" OR qGetRole.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetRole.InactiveCode;
							
				variables.instance.DateCreated = qGetRole.DateCreated;
							
				variables.instance.DateModified = qGetRole.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(RoleID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitRole = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.RoleID) AND variables.instance.RoleID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitRole" datasource="pa_master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitRole" datasource="pa_master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RoleID = qInsertCommitRole.RoleID>
				
				<cfreturn qInsertCommitRole.RoleID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "RoleID,RoleName,SiteID,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "RoleName,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE Role  SET #columnsToUpdate# WHERE RoleID = #trim(variables.instance.RoleID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="RoleName,SiteID,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getRoleQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getRoleQuery">						
		<cfargument name="Fields" required="No" type="String" default="RoleID,RoleName,SiteID,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="RoleName" required="No" type="String" default="">
		<cfargument name="RoleName_IN" required="No" type="String" default="">
		<cfargument name="RoleName_LIKE" required="No" type="String" default="">
		<cfargument name="SiteID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "RoleID,RoleID_IN,RoleID_LIKE,RoleName,RoleName_IN,RoleName_LIKE,SiteID,SiteID_IN,SiteID_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getRoleQuery" datasource="pa_master">
				...
				WHERE 1=1
					...
					<cfif RoleName NEQ "">AND RoleName = '#RoleName#'</cfif>
					<cfif RoleName_IN NEQ "">AND RoleName IN(#trim(RoleName_IN)#)</cfif>
					<cfif RoleName_LIKE NEQ "">AND RoleName LIKE '%#trim(RoleName_LIKE)#%'</cfif>
