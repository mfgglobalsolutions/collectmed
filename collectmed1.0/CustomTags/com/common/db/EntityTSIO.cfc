...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EntityTS table in the dB.                    --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EntityTSTable                                                    --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EntityTSIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.EntityTSID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ENTITYTSID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLR", 4)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ISDEFAULT", 5)>
	<cfset temp = QuerySetCell(defaultsQuery, "IS_NULLABLE", "No", 5)>
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "0", 5)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for EntityTSID.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getEntityTSID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.EntityTSID>
	</cffunction>
	 
	<cffunction name="setEntityTSID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="EntityTSID" required="Yes" type="String">
		
		<cfif EntityTSID NEQ "NULL">	
			
			<cfif arguments.EntityTSID EQ "@@" OR arguments.EntityTSID EQ "">
				
				<cfset arguments.EntityTSID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(EntityTSID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>EntityTSID</strong> (#EntityTSID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.EntityTSID = arguments.EntityTSID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EntityTSIO" output="No">
		
		<cfargument name="EntityTSID" required="yes" type="numeric">		
		<cfset var qGetEntityTS = "">
		
		<cfset preInit(EntityTSID)>
	
		<cfquery name="qGetEntityTS" datasource="#trim(request.datasource)#">
	  		SELECT EntityTSID,EntityID,ColM,ColR,IsDefault,Active,InactiveCode,DateCreated,DateModified
			FROM EntityTS  
			WHERE EntityTSID = #trim(arguments.EntityTSID)# 
		</cfquery>
		...
		<cfscript>
			IF(qGetEntityTS.EntityTSID EQ "" OR qGetEntityTS.EntityTSID EQ "NULL") 
				variables.instance.EntityTSID = "NULL"; 
			ELSE 
				variables.instance.EntityTSID = qGetEntityTS.EntityTSID;
				
			IF(qGetEntityTS.EntityID EQ "" OR qGetEntityTS.EntityID EQ "NULL") 
				variables.instance.EntityID = "NULL"; 
			ELSE 
				variables.instance.EntityID = qGetEntityTS.EntityID;
							
				variables.instance.ColM = qGetEntityTS.ColM;
							
				variables.instance.ColR = qGetEntityTS.ColR;
				
			IF(qGetEntityTS.IsDefault EQ "" OR qGetEntityTS.IsDefault EQ "NULL") 
				variables.instance.IsDefault = "NULL"; 
			ELSE 
				variables.instance.IsDefault = qGetEntityTS.IsDefault;
							
				variables.instance.Active = qGetEntityTS.Active;
				
			IF(qGetEntityTS.InactiveCode EQ "" OR qGetEntityTS.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEntityTS.InactiveCode;
							
				variables.instance.DateCreated = qGetEntityTS.DateCreated;
							
				variables.instance.DateModified = qGetEntityTS.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(EntityTSID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEntityTS = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.EntityTSID) AND variables.instance.EntityTSID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitEntityTS" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEntityTS" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.EntityTSID = qInsertCommitEntityTS.EntityTSID>
				
				<cfreturn qInsertCommitEntityTS.EntityTSID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "EntityTSID,EntityID,ColM,ColR,IsDefault,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "ColM,ColR,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EntityTS  SET #columnsToUpdate# WHERE EntityTSID = #trim(variables.instance.EntityTSID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="EntityID,ColM,ColR,IsDefault,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO EntityTS  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS EntityTSID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEntityTSQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEntityTSQuery">						
		<cfargument name="Fields" required="No" type="String" default="EntityTSID,EntityID,ColM,ColR,IsDefault,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="EntityTSID" required="No" type="String" default="">
		<cfargument name="EntityTSID_IN" required="No" type="String" default="">
		<cfargument name="EntityTSID_LIKE" required="No" type="String" default="">
		<cfargument name="EntityID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "EntityTSID,EntityTSID_IN,EntityTSID_LIKE,EntityID,EntityID_IN,EntityID_LIKE,ColM,ColM_IN,ColM_LIKE,ColR,ColR_IN,ColR_LIKE,IsDefault,IsDefault_IN,IsDefault_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEntityTSQuery" datasource="#trim(request.datasource)#">
				SELECT #trim(Fields)#
				FROM EntityTS   
				WHERE 1=1
					<cfif EntityTSID NEQ "" AND IsNumeric(EntityTSID)>AND EntityTSID = #EntityTSID#</cfif>
					<cfif EntityTSID_IN NEQ "">AND EntityTSID IN(#trim(EntityTSID_IN)#)</cfif>
					<cfif EntityTSID_LIKE NEQ "">AND EntityTSID LIKE '%#trim(EntityTSID_LIKE)#%'</cfif>
