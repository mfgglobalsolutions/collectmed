...
<cfcomponent name="PageRoleIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.PageRoleID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PAGEROLEID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ROLEID", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "SITEID", 4)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for PageRoleID.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getPageRoleID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.PageRoleID>
	</cffunction>
	 
	<cffunction name="setPageRoleID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="PageRoleID" required="Yes" type="String">
		
		<cfif PageRoleID NEQ "NULL">	
			
			<cfif arguments.PageRoleID EQ "@@" OR arguments.PageRoleID EQ "">
				
				<cfset arguments.PageRoleID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(PageRoleID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>PageRoleID</strong> (#PageRoleID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.PageRoleID = arguments.PageRoleID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="InitPageRoleIO" access="public" returntype="PageRoleIO" output="No">
		...
		<cfscript>
			IF(qGetPageRole.PageRoleID EQ "" OR qGetPageRole.PageRoleID EQ "NULL") 
				variables.instance.PageRoleID = "NULL"; 
			ELSE 
				variables.instance.PageRoleID = qGetPageRole.PageRoleID;
				
			IF(qGetPageRole.PageID EQ "" OR qGetPageRole.PageID EQ "NULL") 
				variables.instance.PageID = "NULL"; 
			ELSE 
				variables.instance.PageID = qGetPageRole.PageID;
				
			IF(qGetPageRole.RoleID EQ "" OR qGetPageRole.RoleID EQ "NULL") 
				variables.instance.RoleID = "NULL"; 
			ELSE 
				variables.instance.RoleID = qGetPageRole.RoleID;
				
			IF(qGetPageRole.SiteID EQ "" OR qGetPageRole.SiteID EQ "NULL") 
				variables.instance.SiteID = "NULL"; 
			ELSE 
				variables.instance.SiteID = qGetPageRole.SiteID;
							
				variables.instance.Active = qGetPageRole.Active;
				
			IF(qGetPageRole.InactiveCode EQ "" OR qGetPageRole.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetPageRole.InactiveCode;
							
				variables.instance.DateCreated = qGetPageRole.DateCreated;
							
				variables.instance.DateModified = qGetPageRole.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(PageRoleID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitPageRole = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.PageRoleID) AND variables.instance.PageRoleID  GT 0>	  		
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.PageRoleID = qInsertCommitPageRole.PageRoleID>
				
				<cfreturn qInsertCommitPageRole.PageRoleID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "PageRoleID,PageID,RoleID,SiteID,Active,InactiveCode,DateCreated,DateModified">		
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="PageID,RoleID,SiteID,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getPageRoleQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getPageRoleQuery">						
		<cfargument name="Fields" required="No" type="String" default="PageRoleID,PageID,RoleID,SiteID,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="PageRoleID" required="No" type="String" default="">
		<cfargument name="PageRoleID_IN" required="No" type="String" default="">
		<cfargument name="PageRoleID_LIKE" required="No" type="String" default="">
		<cfargument name="PageID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "PageRoleID,PageRoleID_IN,PageRoleID_LIKE,PageID,PageID_IN,PageID_LIKE,RoleID,RoleID_IN,RoleID_LIKE,SiteID,SiteID_IN,SiteID_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getPageRoleQuery" datasource="pa_master">
				...
				WHERE 1=1
					...
					<cfif PageRoleID_LIKE NEQ "">AND PageRoleID LIKE '%#trim(PageRoleID_LIKE)#%'</cfif>
