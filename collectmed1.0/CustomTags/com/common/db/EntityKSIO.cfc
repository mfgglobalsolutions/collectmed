...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EntityKS table in the dB.                    --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EntityKSTable                                                    --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EntityKSIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.EntityKSID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ENTITYKSID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLM", 2)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLE", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLL", 4)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLI", 5)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLA", 6)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "COLC", 7)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for EntityKSID.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getEntityKSID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.EntityKSID>
	</cffunction>
	 
	<cffunction name="setEntityKSID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="EntityKSID" required="Yes" type="String">
		
		<cfif EntityKSID NEQ "NULL">	
			
			<cfif arguments.EntityKSID EQ "@@" OR arguments.EntityKSID EQ "">
				
				<cfset arguments.EntityKSID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(EntityKSID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>EntityKSID</strong> (#EntityKSID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.EntityKSID = arguments.EntityKSID>			
		<cfreturn true>
	...
	<cffunction name="setColM" access="public" returntype="boolean" output="No">
		...
		<cfif ColM NEQ "NULL">	
			...
			<cfelseif len(ColM) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColM</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<cffunction name="setColE" access="public" returntype="boolean" output="No">
		...
		<cfif ColE NEQ "NULL">	
			...
			<cfelseif len(ColE) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColE</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<cffunction name="setColL" access="public" returntype="boolean" output="No">
		...
		<cfif ColL NEQ "NULL">	
			...
			<cfelseif len(ColL) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColL</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<cffunction name="setColI" access="public" returntype="boolean" output="No">
		...
		<cfif ColI NEQ "NULL">	
			...
			<cfelseif len(ColI) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColI</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<cffunction name="setColA" access="public" returntype="boolean" output="No">
		...
		<cfif ColA NEQ "NULL">	
			...
			<cfelseif len(ColA) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColA</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<cffunction name="setColC" access="public" returntype="boolean" output="No">
		...
		<cfif ColC NEQ "NULL">	
			...
			<cfelseif len(ColC) GT 6>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ColC</strong> cannot be greater than 6 character(s).">
			</cfif>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EntityKSIO" output="No">
		...
		<cfscript>
			IF(qGetEntityKS.EntityKSID EQ "" OR qGetEntityKS.EntityKSID EQ "NULL") 
				variables.instance.EntityKSID = "NULL"; 
			ELSE 
				variables.instance.EntityKSID = qGetEntityKS.EntityKSID;
							
				variables.instance.ColM = qGetEntityKS.ColM;
							
				variables.instance.ColE = qGetEntityKS.ColE;
							
				variables.instance.ColL = qGetEntityKS.ColL;
							
				variables.instance.ColI = qGetEntityKS.ColI;
							
				variables.instance.ColA = qGetEntityKS.ColA;
							
				variables.instance.ColC = qGetEntityKS.ColC;
							
				variables.instance.Active = qGetEntityKS.Active;
				
			IF(qGetEntityKS.InactiveCode EQ "" OR qGetEntityKS.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEntityKS.InactiveCode;
							
				variables.instance.DateCreated = qGetEntityKS.DateCreated;
							
				variables.instance.DateModified = qGetEntityKS.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(EntityKSID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEntityKS = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.EntityKSID) AND variables.instance.EntityKSID  GT 0>	  		
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.EntityKSID = qInsertCommitEntityKS.EntityKSID>
				
				<cfreturn qInsertCommitEntityKS.EntityKSID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "EntityKSID,ColM,ColE,ColL,ColI,ColA,ColC,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "ColM,ColE,ColL,ColI,ColA,ColC,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EntityKS  SET #columnsToUpdate# WHERE EntityKSID = #trim(variables.instance.EntityKSID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="ColM,ColE,ColL,ColI,ColA,ColC,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEntityKSQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEntityKSQuery">						
		<cfargument name="Fields" required="No" type="String" default="EntityKSID,ColM,ColE,ColL,ColI,ColA,ColC,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="EntityKSID" required="No" type="String" default="">
		<cfargument name="EntityKSID_IN" required="No" type="String" default="">
		<cfargument name="EntityKSID_LIKE" required="No" type="String" default="">
		<cfargument name="ColM" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "EntityKSID,EntityKSID_IN,EntityKSID_LIKE,ColM,ColM_IN,ColM_LIKE,ColE,ColE_IN,ColE_LIKE,ColL,ColL_IN,ColL_LIKE,ColI,ColI_IN,ColI_LIKE,ColA,ColA_IN,ColA_LIKE,ColC,ColC_IN,ColC_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEntityKSQuery" datasource="pa_master">
				...
				WHERE 1=1
					...
					<cfif EntityKSID_LIKE NEQ "">AND EntityKSID LIKE '%#trim(EntityKSID_LIKE)#%'</cfif>
