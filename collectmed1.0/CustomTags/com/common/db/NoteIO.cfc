...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Note table in the dB.                        --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
...
<cfcomponent name="NoteIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.NoteID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "NOTEID", 1)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for NoteID.                                                 --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getNoteID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.NoteID>
	</cffunction>
	 
	<cffunction name="setNoteID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="NoteID" required="Yes" type="String">
		
		<cfif NoteID NEQ "NULL">	
			
			<cfif arguments.NoteID EQ "@@" OR arguments.NoteID EQ "">
				
				<cfset arguments.NoteID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(NoteID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>NoteID</strong> (#NoteID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.NoteID = arguments.NoteID>			
		<cfreturn true>
	...
	<cffunction name="InitNoteIO" access="public" returntype="NoteIO" output="No">
		...
		<cfscript>
			IF(qGetNote.NoteID EQ "" OR qGetNote.NoteID EQ "NULL") 
				variables.instance.NoteID = "NULL"; 
			ELSE 
				variables.instance.NoteID = qGetNote.NoteID;
				
			IF(qGetNote.ClientID EQ "" OR qGetNote.ClientID EQ "NULL") 
				variables.instance.ClientID = "NULL"; 
			ELSE 
				variables.instance.ClientID = qGetNote.ClientID;
				
			IF(qGetNote.ObjectID EQ "" OR qGetNote.ObjectID EQ "NULL") 
				variables.instance.ObjectID = "NULL"; 
			ELSE 
				variables.instance.ObjectID = qGetNote.ObjectID;
				
			IF(qGetNote.InstanceID EQ "" OR qGetNote.InstanceID EQ "NULL") 
				variables.instance.InstanceID = "NULL"; 
			ELSE 
				variables.instance.InstanceID = qGetNote.InstanceID;
							
				variables.instance.Note = qGetNote.Note;
							
				variables.instance.Active = qGetNote.Active;
				
			IF(qGetNote.InactiveCode EQ "" OR qGetNote.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetNote.InactiveCode;
							
				variables.instance.DateCreated = qGetNote.DateCreated;
							
				variables.instance.DateModified = qGetNote.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(NoteID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitNote = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.NoteID) AND variables.instance.NoteID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitNote" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitNote" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.NoteID = qInsertCommitNote.NoteID>
				
				<cfreturn qInsertCommitNote.NoteID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "NoteID,ClientID,ObjectID,InstanceID,Note,Active,InactiveCode,DateCreated,DateModified">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE note  SET #columnsToUpdate# WHERE NoteID = #trim(variables.instance.NoteID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="ClientID,ObjectID,InstanceID,Note,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getNoteQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getNoteQuery">						
		<cfargument name="Fields" required="No" type="String" default="NoteID,ClientID,ObjectID,InstanceID,Note,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="NoteID" required="No" type="String" default="">
		<cfargument name="NoteID_IN" required="No" type="String" default="">
		<cfargument name="NoteID_LIKE" required="No" type="String" default="">
		<cfargument name="ClientID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getNoteQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif NoteID_LIKE NEQ "">AND NoteID LIKE '%#trim(NoteID_LIKE)#%'</cfif>
