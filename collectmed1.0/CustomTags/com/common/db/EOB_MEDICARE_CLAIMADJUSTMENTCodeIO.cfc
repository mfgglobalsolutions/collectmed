...
<!--- NAME:                                                                          --->
<!--- @@Name@@                                                                       --->
<!--- EOB_MEDICARE_CLAIMADJUSTMENTCodeIO.cfc                                         --->
<!--- @@Name@@                                                                       --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- CATEGORY:                                                                      --->
...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_MEDICARE_CLAIMADJUSTMENTCode table in    --->
<!--- the dB.                                                                        --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_MEDICARE_CLAIMADJUSTMENTCodeTable                            --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_MEDICARE_CLAIMADJUSTMENTCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_MEDICARE_CLAIMADJUSTMENTCodeIO" output="No">
		...
		<cfquery name="qGetEOB_MEDICARE_CLAIMADJUSTMENTCode" datasource="pa_master">
		...
		<cfscript>
			IF(qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.RecordID EQ "" OR qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.RecordID;
							
				variables.instance.Code = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.Code;
							
				variables.instance.Description = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.Description;
							
				variables.instance.Active = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.Active;
				
			IF(qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.InactiveCode EQ "" OR qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_MEDICARE_CLAIMADJUSTMENTCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEOB_MEDICARE_CLAIMADJUSTMENTCode = "">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_MEDICARE_CLAIMADJUSTMENTCode" datasource="pa_master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_MEDICARE_CLAIMADJUSTMENTCode" datasource="pa_master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_MEDICARE_CLAIMADJUSTMENTCode.RecordID>
				
				<cfreturn qInsertCommitEOB_MEDICARE_CLAIMADJUSTMENTCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "RecordID,Code,Description,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "Code,Description,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE eob_medicare_claimadjustmentcode SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="Code,Description,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO eob_medicare_claimadjustmentcode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_MEDICARE_CLAIMADJUSTMENTCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_MEDICARE_CLAIMADJUSTMENTCodeQuery">						
		<cfargument name="Fields" required="No" type="String" default="RecordID,Code,Description,Active,InactiveCode,DateCreated,DateModified">	
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "RecordID,RecordID_IN,RecordID_LIKE,Code,Code_IN,Code_LIKE,Description,Description_IN,Description_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
			...
			<cfloop from="1" to="#ArrayLen(keysToStruct)#" index="i">
				...
				<cfif NOT ListFindNoCase("QueryName|Fields|OrderBy|GroupBy", keysToStruct[i], "|") AND Find("_IN", keysToStruct[i]) EQ 0 AND Find("_LIKE", keysToStruct[i]) EQ 0>
					...
					<cfif NOT ListFindNoCase(availableColumns, keysToStruct[i])>
						<cf_gcGeneralErrorTemplate				
							invisible="EOB_MEDICARE_CLAIMADJUSTMENTCode"
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_MEDICARE_CLAIMADJUSTMENTCodeQuery" datasource="pa_master">
				SELECT #trim(Fields)#
				FROM eob_medicare_claimadjustmentcode   
				WHERE 1=1
			...
			<cfreturn getEOB_MEDICARE_CLAIMADJUSTMENTCodeQuery>
