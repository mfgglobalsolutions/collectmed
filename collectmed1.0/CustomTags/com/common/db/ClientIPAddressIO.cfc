...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the ClientIPAddress table in the dB.             --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_ClientIPAddressTable                                             --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="ClientIPAddressIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.ClientIPAddressID = "NULL";
			variables.instance.IPAddressFrom = "";
			variables.instance.IPAddressTo = "";
			variables.instance.UsersID = "NULL";
			variables.instance.AccessTimeIn = "";
			variables.instance.AccessTimeOut = "";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CLIENTIPADDRESSID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "IPADDRESSFROM", 2)>
	<cfset temp = QuerySetCell(defaultsQuery, "IS_NULLABLE", "No", 2)>
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "", 2)>
	<cfset temp = QuerySetCell(defaultsQuery, "DATA_TYPE", "char", 2)>
	
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "IPADDRESSTO", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ACCESSTIMEIN", 5)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ACCESSTIMEOUT", 6)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for ClientIPAddressID.                                      --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getClientIPAddressID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ClientIPAddressID>
	</cffunction>
	 
	<cffunction name="setClientIPAddressID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ClientIPAddressID" required="Yes" type="String">
		
		<cfif ClientIPAddressID NEQ "NULL">	
			
			<cfif arguments.ClientIPAddressID EQ "@@" OR arguments.ClientIPAddressID EQ "">
				
				<cfset arguments.ClientIPAddressID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(ClientIPAddressID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ClientIPAddressID</strong> (#ClientIPAddressID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.ClientIPAddressID = arguments.ClientIPAddressID>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for IPAddressFrom.                                          --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getIPAddressFrom" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.IPAddressFrom>
	</cffunction>
	 
	<cffunction name="setIPAddressFrom" access="public" returntype="boolean" output="No">
 		
		<cfargument name="IPAddressFrom" required="Yes" type="String">
		
		<cfif IPAddressFrom NEQ "NULL">	
			
			<cfif arguments.IPAddressFrom EQ "@@" OR arguments.IPAddressFrom EQ "">
				
				<cfset arguments.IPAddressFrom = "NULL">
						
			<cfelseif NOT REQUEST.ischarDataType(IPAddressFrom)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>IPAddressFrom</strong> (#IPAddressFrom#) is not of type <strong>char</strong>.">	
			<cfelseif len(IPAddressFrom) GT 15>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>IPAddressFrom</strong> cannot be greater than 15 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.IPAddressFrom = arguments.IPAddressFrom>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "IPAddressFrom")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for IPAddressTo.                                            --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getIPAddressTo" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.IPAddressTo>
	</cffunction>
	 
	<cffunction name="setIPAddressTo" access="public" returntype="boolean" output="No">
 		
		<cfargument name="IPAddressTo" required="Yes" type="String">
		
		<cfif IPAddressTo NEQ "NULL">	
			
			<cfif arguments.IPAddressTo EQ "@@" OR arguments.IPAddressTo EQ "">
				
				<cfset arguments.IPAddressTo = "NULL">
						
			<cfelseif NOT REQUEST.ischarDataType(IPAddressTo)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>IPAddressTo</strong> (#IPAddressTo#) is not of type <strong>char</strong>.">	
			<cfelseif len(IPAddressTo) GT 15>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>IPAddressTo</strong> cannot be greater than 15 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.IPAddressTo = arguments.IPAddressTo>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "IPAddressTo")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for AccessTimeIn.                                           --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getAccessTimeIn" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.AccessTimeIn>
	</cffunction>
	 
	<cffunction name="setAccessTimeIn" access="public" returntype="boolean" output="No">
 		
		<cfargument name="AccessTimeIn" required="Yes" type="String">
		
		<cfif AccessTimeIn NEQ "NULL" AND AccessTimeIn NEQ "NOW">	
			
			<cfif arguments.AccessTimeIn EQ "@@" OR arguments.AccessTimeIn EQ "">
				
				<cfset arguments.AccessTimeIn = "NULL">
						
			<cfelseif NOT REQUEST.isdatetimeDataType(AccessTimeIn)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>AccessTimeIn</strong> (#AccessTimeIn#) is not of type <strong>datetime</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.AccessTimeIn = arguments.AccessTimeIn>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "AccessTimeIn")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for AccessTimeOut.                                          --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getAccessTimeOut" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.AccessTimeOut>
	</cffunction>
	 
	<cffunction name="setAccessTimeOut" access="public" returntype="boolean" output="No">
 		
		<cfargument name="AccessTimeOut" required="Yes" type="String">
		
		<cfif AccessTimeOut NEQ "NULL" AND AccessTimeOut NEQ "NOW">	
			
			<cfif arguments.AccessTimeOut EQ "@@" OR arguments.AccessTimeOut EQ "">
				
				<cfset arguments.AccessTimeOut = "NULL">
						
			<cfelseif NOT REQUEST.isdatetimeDataType(AccessTimeOut)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>AccessTimeOut</strong> (#AccessTimeOut#) is not of type <strong>datetime</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.AccessTimeOut = arguments.AccessTimeOut>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "AccessTimeOut")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="IPAddressFrom" required="No" type="String" default="">
		<cfargument name="IPAddressTo" required="No" type="String" default="">
		<cfargument name="UsersID" required="No" type="String" default="">
		<cfargument name="AccessTimeIn" required="No" type="String" default="">
		<cfargument name="AccessTimeOut" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="ClientIPAddressIO" output="No">
		
		<cfargument name="ClientIPAddressID" required="yes" type="numeric">		
		<cfset var qGetClientIPAddress = "">
		
		<cfset preInit(ClientIPAddressID)>
	
		<cfquery name="qGetClientIPAddress" datasource="#request.datasource#">
	  		SELECT ClientIPAddressID,IPAddressFrom,IPAddressTo,UsersID,AccessTimeIn,AccessTimeOut,Active,InactiveCode,DateCreated,DateModified
			FROM ClientIPAddress  
			WHERE ClientIPAddressID = #trim(arguments.ClientIPAddressID)# 
		</cfquery>
		
		<cfif qGetClientIPAddress.Recordcount LTE 0>
			<cf_gcGeneralErrorTemplate				
		...
		<cfscript>
			IF(qGetClientIPAddress.ClientIPAddressID EQ "" OR qGetClientIPAddress.ClientIPAddressID EQ "NULL") 
				variables.instance.ClientIPAddressID = "NULL"; 
			ELSE 
				variables.instance.ClientIPAddressID = qGetClientIPAddress.ClientIPAddressID;
							
				variables.instance.IPAddressFrom = qGetClientIPAddress.IPAddressFrom;
							
				variables.instance.IPAddressTo = qGetClientIPAddress.IPAddressTo;
				
			IF(qGetClientIPAddress.UsersID EQ "" OR qGetClientIPAddress.UsersID EQ "NULL") 
				variables.instance.UsersID = "NULL"; 
			ELSE 
				variables.instance.UsersID = qGetClientIPAddress.UsersID;
							
				variables.instance.AccessTimeIn = qGetClientIPAddress.AccessTimeIn;
							
				variables.instance.AccessTimeOut = qGetClientIPAddress.AccessTimeOut;
							
				variables.instance.Active = qGetClientIPAddress.Active;
				
			IF(qGetClientIPAddress.InactiveCode EQ "" OR qGetClientIPAddress.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetClientIPAddress.InactiveCode;
							
				variables.instance.DateCreated = qGetClientIPAddress.DateCreated;
							
				variables.instance.DateModified = qGetClientIPAddress.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(ClientIPAddressID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitClientIPAddress = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.ClientIPAddressID) AND variables.instance.ClientIPAddressID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitClientIPAddress" datasource="#request.datasource#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitClientIPAddress" datasource="#request.datasource#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.ClientIPAddressID = qInsertCommitClientIPAddress.ClientIPAddressID>
				
				<cfreturn qInsertCommitClientIPAddress.ClientIPAddressID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "ClientIPAddressID,IPAddressFrom,IPAddressTo,UsersID,AccessTimeIn,AccessTimeOut,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "IPAddressFrom,IPAddressTo,Active">	
		...
		<!-------------------------------------------------------------------------------------->
			<cfset dateColumnList = "AccessTimeIn,AccessTimeOut,DateCreated,DateModified">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="IPAddressFrom,IPAddressTo,UsersID,AccessTimeIn,AccessTimeOut,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getClientIPAddressQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getClientIPAddressQuery">						
		<cfargument name="Fields" required="No" type="String" default="ClientIPAddressID,IPAddressFrom,IPAddressTo,UsersID,AccessTimeIn,AccessTimeOut,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="ClientIPAddressID" required="No" type="String" default="">
		<cfargument name="ClientIPAddressID_IN" required="No" type="String" default="">
		<cfargument name="ClientIPAddressID_LIKE" required="No" type="String" default="">
		<cfargument name="IPAddressFrom" required="No" type="String" default="">
		<cfargument name="IPAddressFrom_IN" required="No" type="String" default="">
		<cfargument name="IPAddressFrom_LIKE" required="No" type="String" default="">
		<cfargument name="IPAddressTo" required="No" type="String" default="">
		<cfargument name="IPAddressTo_IN" required="No" type="String" default="">
		<cfargument name="IPAddressTo_LIKE" required="No" type="String" default="">
		<cfargument name="UsersID" required="No" type="String" default="">
		<cfargument name="UsersID_IN" required="No" type="String" default="">
		<cfargument name="UsersID_LIKE" required="No" type="String" default="">
		<cfargument name="AccessTimeIn" required="No" type="String" default="">
		<cfargument name="AccessTimeIn_IN" required="No" type="String" default="">
		<cfargument name="AccessTimeIn_LIKE" required="No" type="String" default="">
		<cfargument name="AccessTimeOut" required="No" type="String" default="">
		<cfargument name="AccessTimeOut_IN" required="No" type="String" default="">
		<cfargument name="AccessTimeOut_LIKE" required="No" type="String" default="">
		<cfargument name="Active" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "ClientIPAddressID,ClientIPAddressID_IN,ClientIPAddressID_LIKE,IPAddressFrom,IPAddressFrom_IN,IPAddressFrom_LIKE,IPAddressTo,IPAddressTo_IN,IPAddressTo_LIKE,UsersID,UsersID_IN,UsersID_LIKE,AccessTimeIn,AccessTimeIn_IN,AccessTimeIn_LIKE,AccessTimeOut,AccessTimeOut_IN,AccessTimeOut_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getClientIPAddressQuery" datasource="#trim(request.DataSource)#" cachedwithin="#CreateTimeSpan(0,0,0,0)#">
				SELECT #trim(Fields)#
				FROM ClientIPAddress  WHERE 1=1
					<cfif ClientIPAddressID NEQ "" AND IsNumeric(ClientIPAddressID)>AND ClientIPAddressID = #ClientIPAddressID#</cfif>
					<cfif ClientIPAddressID_IN NEQ "">AND ClientIPAddressID IN(#trim(ClientIPAddressID_IN)#)</cfif>
					<cfif ClientIPAddressID_LIKE NEQ "">AND ClientIPAddressID LIKE '%#trim(ClientIPAddressID_LIKE)#%'</cfif>
					<cfif IPAddressFrom NEQ "">AND IPAddressFrom = '#IPAddressFrom#'</cfif>
					<cfif IPAddressFrom_IN NEQ "">AND IPAddressFrom IN(#trim(IPAddressFrom_IN)#)</cfif>
					<cfif IPAddressFrom_LIKE NEQ "">AND IPAddressFrom LIKE '%#trim(IPAddressFrom_LIKE)#%'</cfif>
					<cfif IPAddressTo NEQ "">AND IPAddressTo = '#IPAddressTo#'</cfif>
					<cfif IPAddressTo_IN NEQ "">AND IPAddressTo IN(#trim(IPAddressTo_IN)#)</cfif>
					<cfif IPAddressTo_LIKE NEQ "">AND IPAddressTo LIKE '%#trim(IPAddressTo_LIKE)#%'</cfif>
					...
					<cfif AccessTimeIn NEQ "">AND AccessTimeIn = '#AccessTimeIn#'</cfif>
					<cfif AccessTimeIn_IN NEQ "">AND AccessTimeIn IN(#trim(AccessTimeIn_IN)#)</cfif>
					<cfif AccessTimeIn_LIKE NEQ "">AND AccessTimeIn LIKE '%#trim(AccessTimeIn_LIKE)#%'</cfif>
					<cfif AccessTimeOut NEQ "">AND AccessTimeOut = '#AccessTimeOut#'</cfif>
					<cfif AccessTimeOut_IN NEQ "">AND AccessTimeOut IN(#trim(AccessTimeOut_IN)#)</cfif>
					<cfif AccessTimeOut_LIKE NEQ "">AND AccessTimeOut LIKE '%#trim(AccessTimeOut_LIKE)#%'</cfif>
			...
			<cfreturn getClientIPAddressQuery>
