...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Phone table in the dB.                       --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_PhoneTable                                                       --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="PhoneIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.PhoneID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PHONEID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PHONETYPEID", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PHONENUMBER", 4)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PHONEEXTENSION", 5)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for PhoneID.                                                --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getPhoneID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.PhoneID>
	</cffunction>
	 
	<cffunction name="setPhoneID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="PhoneID" required="Yes" type="String">
		
		<cfif PhoneID NEQ "NULL">	
			
			<cfif arguments.PhoneID EQ "@@" OR arguments.PhoneID EQ "">
				
				<cfset arguments.PhoneID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(PhoneID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>PhoneID</strong> (#PhoneID#) is not of type <strong>int</strong>.">
			</cfif>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for PhoneTypeID.                                            --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getPhoneTypeID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.PhoneTypeID>
	</cffunction>
	 
	<cffunction name="setPhoneTypeID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="PhoneTypeID" required="Yes" type="String">
		
		<cfif PhoneTypeID NEQ "NULL">	
			
			<cfif arguments.PhoneTypeID EQ "@@" OR arguments.PhoneTypeID EQ "">
				
				<cfset arguments.PhoneTypeID = "NULL">
						
			<cfelseif NOT REQUEST.ischarDataType(PhoneTypeID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>PhoneTypeID</strong> (#PhoneTypeID#) is not of type <strong>char</strong>.">	
			<cfelseif len(PhoneTypeID) GT 10>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>PhoneTypeID</strong> cannot be greater than 10 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.PhoneTypeID = arguments.PhoneTypeID>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "PhoneTypeID")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for PhoneNumber.                                            --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getPhoneNumber" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.PhoneNumber>
	</cffunction>
	 
	<cffunction name="setPhoneNumber" access="public" returntype="boolean" output="No">
 		
		<cfargument name="PhoneNumber" required="Yes" type="String">
		
		<cfif PhoneNumber NEQ "NULL">	
			
			<cfif arguments.PhoneNumber EQ "@@" OR arguments.PhoneNumber EQ "">
				
				<cfset arguments.PhoneNumber = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(PhoneNumber)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>PhoneNumber</strong> (#PhoneNumber#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(PhoneNumber) GT 100>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>PhoneNumber</strong> cannot be greater than 100 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.PhoneNumber = arguments.PhoneNumber>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "PhoneNumber")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for PhoneExtension.                                         --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getPhoneExtension" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.PhoneExtension>
	</cffunction>
	 
	<cffunction name="setPhoneExtension" access="public" returntype="boolean" output="No">
 		
		<cfargument name="PhoneExtension" required="Yes" type="String">
		
		<cfif PhoneExtension NEQ "NULL">	
			
			<cfif arguments.PhoneExtension EQ "@@" OR arguments.PhoneExtension EQ "">
				
				<cfset arguments.PhoneExtension = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(PhoneExtension)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>PhoneExtension</strong> (#PhoneExtension#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(PhoneExtension) GT 50>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>PhoneExtension</strong> cannot be greater than 50 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.PhoneExtension = arguments.PhoneExtension>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "PhoneExtension")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="PhoneTypeID" required="No" type="String" default="">
		<cfargument name="PhoneNumber" required="No" type="String" default="">
		<cfargument name="PhoneExtension" required="No" type="String" default="">
	...
	<cffunction name="Init" access="public" returntype="PhoneIO" output="No">
		...
		<cfset preInit(PhoneID)>
	
		<cfquery name="qGetPhone" datasource="#trim(request.datasource)#">
	  		SELECT PhoneID,SiteID,PhoneTypeID,PhoneNumber,PhoneExtension,Active,InactiveCode,DateCreated,DateModified
			FROM phone  
			WHERE PhoneID = #trim(arguments.PhoneID)# 
		</cfquery>
		...
		<cfscript>
			IF(qGetPhone.PhoneID EQ "" OR qGetPhone.PhoneID EQ "NULL") 
				variables.instance.PhoneID = "NULL"; 
			ELSE 
				variables.instance.PhoneID = qGetPhone.PhoneID;
				
			IF(qGetPhone.SiteID EQ "" OR qGetPhone.SiteID EQ "NULL") 
				variables.instance.SiteID = "NULL"; 
			ELSE 
				variables.instance.SiteID = qGetPhone.SiteID;
							
				variables.instance.PhoneTypeID = qGetPhone.PhoneTypeID;
							
				variables.instance.PhoneNumber = qGetPhone.PhoneNumber;
							
				variables.instance.PhoneExtension = qGetPhone.PhoneExtension;
							
				variables.instance.Active = qGetPhone.Active;
				
			IF(qGetPhone.InactiveCode EQ "" OR qGetPhone.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetPhone.InactiveCode;
							
				variables.instance.DateCreated = qGetPhone.DateCreated;
							
				variables.instance.DateModified = qGetPhone.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(PhoneID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitPhone = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.PhoneID) AND variables.instance.PhoneID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitPhone" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitPhone" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.PhoneID = qInsertCommitPhone.PhoneID>
				
				<cfreturn qInsertCommitPhone.PhoneID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "PhoneID,SiteID,PhoneTypeID,PhoneNumber,PhoneExtension,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "PhoneTypeID,PhoneNumber,PhoneExtension,Active">	
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE Phone  SET #columnsToUpdate# WHERE PhoneID = #trim(variables.instance.PhoneID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="SiteID,PhoneTypeID,PhoneNumber,PhoneExtension,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO phone  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS PhoneID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getPhoneQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getPhoneQuery">						
		<cfargument name="Fields" required="No" type="String" default="PhoneID,SiteID,PhoneTypeID,PhoneNumber,PhoneExtension,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="PhoneID" required="No" type="String" default="">
		<cfargument name="PhoneID_IN" required="No" type="String" default="">
		<cfargument name="PhoneID_LIKE" required="No" type="String" default="">
		<cfargument name="SiteID" required="No" type="String" default="">
		<cfargument name="SiteID_IN" required="No" type="String" default="">
		<cfargument name="SiteID_LIKE" required="No" type="String" default="">
		<cfargument name="PhoneTypeID" required="No" type="String" default="">
		<cfargument name="PhoneTypeID_IN" required="No" type="String" default="">
		<cfargument name="PhoneTypeID_LIKE" required="No" type="String" default="">
		<cfargument name="PhoneNumber" required="No" type="String" default="">
		<cfargument name="PhoneNumber_IN" required="No" type="String" default="">
		<cfargument name="PhoneNumber_LIKE" required="No" type="String" default="">
		<cfargument name="PhoneExtension" required="No" type="String" default="">
		<cfargument name="PhoneExtension_IN" required="No" type="String" default="">
		<cfargument name="PhoneExtension_LIKE" required="No" type="String" default="">
		<cfargument name="Active" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "PhoneID,PhoneID_IN,PhoneID_LIKE,SiteID,SiteID_IN,SiteID_LIKE,PhoneTypeID,PhoneTypeID_IN,PhoneTypeID_LIKE,PhoneNumber,PhoneNumber_IN,PhoneNumber_LIKE,PhoneExtension,PhoneExtension_IN,PhoneExtension_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getPhoneQuery" datasource="#trim(request.datasource)#">
				SELECT #trim(Fields)#
				FROM phone   
				WHERE 1=1
					<cfif PhoneID NEQ "" AND IsNumeric(PhoneID)>AND PhoneID = #PhoneID#</cfif>
					<cfif PhoneID_IN NEQ "">AND PhoneID IN(#trim(PhoneID_IN)#)</cfif>
					<cfif PhoneID_LIKE NEQ "">AND PhoneID LIKE '%#trim(PhoneID_LIKE)#%'</cfif>
					...
					<cfif PhoneTypeID NEQ "">AND PhoneTypeID = '#PhoneTypeID#'</cfif>
					<cfif PhoneTypeID_IN NEQ "">AND PhoneTypeID IN(#trim(PhoneTypeID_IN)#)</cfif>
					<cfif PhoneTypeID_LIKE NEQ "">AND PhoneTypeID LIKE '%#trim(PhoneTypeID_LIKE)#%'</cfif>
					<cfif PhoneNumber NEQ "">AND PhoneNumber = '#PhoneNumber#'</cfif>
					<cfif PhoneNumber_IN NEQ "">AND PhoneNumber IN(#trim(PhoneNumber_IN)#)</cfif>
					<cfif PhoneNumber_LIKE NEQ "">AND PhoneNumber LIKE '%#trim(PhoneNumber_LIKE)#%'</cfif>
					<cfif PhoneExtension NEQ "">AND PhoneExtension = '#PhoneExtension#'</cfif>
					<cfif PhoneExtension_IN NEQ "">AND PhoneExtension IN(#trim(PhoneExtension_IN)#)</cfif>
					<cfif PhoneExtension_LIKE NEQ "">AND PhoneExtension LIKE '%#trim(PhoneExtension_LIKE)#%'</cfif>
