...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Reminder table in the dB.                    --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_ReminderTable                                                    --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "REMINDERID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "OBJECTID", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "INSTANCEID", 4)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "SHOWDATE", 5)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DISMISSED", 6)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "NOTE", 7)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for ReminderID.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getReminderID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ReminderID>
	</cffunction>
	 
	<cffunction name="setReminderID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ReminderID" required="Yes" type="String">
		
		<cfif ReminderID NEQ "NULL">	
			
			<cfif arguments.ReminderID EQ "@@" OR arguments.ReminderID EQ "">
				
				<cfset arguments.ReminderID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(ReminderID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ReminderID</strong> (#ReminderID#) is not of type <strong>int</strong>.">
			</cfif>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for showDate.                                               --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getShowDate" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ShowDate>
	</cffunction>
	 
	<cffunction name="setShowDate" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ShowDate" required="Yes" type="String">
		
		<cfif ShowDate NEQ "NULL" AND ShowDate NEQ "NOW">	
			
			<cfif arguments.showDate EQ "@@" OR arguments.showDate EQ "">
				
				<cfset arguments.showDate = "NULL">
						
			<cfelseif NOT REQUEST.isdatetimeDataType(ShowDate)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ShowDate</strong> (#ShowDate#) is not of type <strong>datetime</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.ShowDate = arguments.ShowDate>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "ShowDate")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for Dismissed.                                              --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getDismissed" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.Dismissed>
	</cffunction>
	 
	<cffunction name="setDismissed" access="public" returntype="boolean" output="No">
 		
		<cfargument name="Dismissed" required="Yes" type="String">
		
		<cfif Dismissed NEQ "NULL">	
			
			<cfif arguments.Dismissed EQ "@@" OR arguments.Dismissed EQ "">
				
				<cfset arguments.Dismissed = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(Dismissed)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>Dismissed</strong> (#Dismissed#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.Dismissed = arguments.Dismissed>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "Dismissed")>			
		<cfreturn true>
	...
	<cffunction name="setNote" access="public" returntype="boolean" output="No">
		...
		<cfif Note NEQ "NULL">	
			...
			<cfelseif len(Note) GT 4000>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>Note</strong> cannot be greater than 4000 character(s).">
			</cfif>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="showDate" required="No" type="String" default="">
		<cfargument name="Dismissed" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="ReminderIO" output="No">
		
		<cfargument name="ReminderID" required="yes" type="numeric">		
		<cfset var qGetReminder = "">
		
		<cfset preInit(ReminderID)>
		...
		<cfscript>
			IF(qGetReminder.ReminderID EQ "" OR qGetReminder.ReminderID EQ "NULL") 
				variables.instance.ReminderID = "NULL"; 
			ELSE 
				variables.instance.ReminderID = qGetReminder.ReminderID;
				
			IF(qGetReminder.UsersID EQ "" OR qGetReminder.UsersID EQ "NULL") 
				variables.instance.UsersID = "NULL"; 
			ELSE 
				variables.instance.UsersID = qGetReminder.UsersID;
				
			IF(qGetReminder.ObjectID EQ "" OR qGetReminder.ObjectID EQ "NULL") 
				variables.instance.ObjectID = "NULL"; 
			ELSE 
				variables.instance.ObjectID = qGetReminder.ObjectID;
				
			IF(qGetReminder.InstanceID EQ "" OR qGetReminder.InstanceID EQ "NULL") 
				variables.instance.InstanceID = "NULL"; 
			ELSE 
				variables.instance.InstanceID = qGetReminder.InstanceID;
							
				variables.instance.showDate = qGetReminder.showDate;
				
			IF(qGetReminder.Dismissed EQ "" OR qGetReminder.Dismissed EQ "NULL") 
				variables.instance.Dismissed = "NULL"; 
			ELSE 
				variables.instance.Dismissed = qGetReminder.Dismissed;
							
				variables.instance.Note = qGetReminder.Note;
							
				variables.instance.Active = qGetReminder.Active;
				
			IF(qGetReminder.InactiveCode EQ "" OR qGetReminder.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetReminder.InactiveCode;
							
				variables.instance.DateCreated = qGetReminder.DateCreated;
							
				variables.instance.DateModified = qGetReminder.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(ReminderID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitReminder = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.ReminderID) AND variables.instance.ReminderID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitReminder" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitReminder" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.ReminderID = qInsertCommitReminder.ReminderID>
				
				<cfreturn qInsertCommitReminder.ReminderID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "ReminderID,UsersID,ObjectID,InstanceID,showDate,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified">		
		...
		<!-------------------------------------------------------------------------------------->
			<cfset dateColumnList = "showDate,DateCreated,DateModified">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="UsersID,ObjectID,InstanceID,showDate,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getReminderQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getReminderQuery">						
		<cfargument name="Fields" required="No" type="String" default="ReminderID,UsersID,ObjectID,InstanceID,showDate,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="ReminderID" required="No" type="String" default="">
		<cfargument name="ReminderID_IN" required="No" type="String" default="">
		<cfargument name="ReminderID_LIKE" required="No" type="String" default="">
		<cfargument name="UsersID" required="No" type="String" default="">
		...
		<cfargument name="showDate" required="No" type="String" default="">
		<cfargument name="showDate_IN" required="No" type="String" default="">
		<cfargument name="showDate_LIKE" required="No" type="String" default="">
		<cfargument name="Dismissed" required="No" type="String" default="">
		<cfargument name="Dismissed_IN" required="No" type="String" default="">
		<cfargument name="Dismissed_LIKE" required="No" type="String" default="">
		<cfargument name="Note" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "ReminderID,ReminderID_IN,ReminderID_LIKE,UsersID,UsersID_IN,UsersID_LIKE,ObjectID,ObjectID_IN,ObjectID_LIKE,InstanceID,InstanceID_IN,InstanceID_LIKE,showDate,showDate_IN,showDate_LIKE,Dismissed,Dismissed_IN,Dismissed_LIKE,Note,Note_IN,Note_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getReminderQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif ReminderID_LIKE NEQ "">AND ReminderID LIKE '%#trim(ReminderID_LIKE)#%'</cfif>
					...
					<cfif showDate NEQ "">AND showDate = '#showDate#'</cfif>
					<cfif showDate_IN NEQ "">AND showDate IN(#trim(showDate_IN)#)</cfif>
					<cfif showDate_LIKE NEQ "">AND showDate LIKE '%#trim(showDate_LIKE)#%'</cfif>
					<cfif Dismissed NEQ "" AND IsNumeric(Dismissed)>AND Dismissed = #Dismissed#</cfif>
					<cfif Dismissed_IN NEQ "">AND Dismissed IN(#trim(Dismissed_IN)#)</cfif>
					<cfif Dismissed_LIKE NEQ "">AND Dismissed LIKE '%#trim(Dismissed_LIKE)#%'</cfif>
