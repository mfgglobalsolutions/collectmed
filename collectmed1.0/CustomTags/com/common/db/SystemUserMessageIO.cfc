...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the SystemUserMessage table in the dB.           --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_SystemUserMessageTable                                           --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="SystemUserMessageIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.SystemUserMessageID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "SYSTEMUSERMESSAGEID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DISMISSED", 3)>
	<cfset temp = QuerySetCell(defaultsQuery, "IS_NULLABLE", "No", 3)>
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "0", 3)>
	<cfset temp = QuerySetCell(defaultsQuery, "DATA_TYPE", "int", 3)>
	
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "NOTE", 4)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for SystemUserMessageID.                                    --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getSystemUserMessageID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.SystemUserMessageID>
	</cffunction>
	 
	<cffunction name="setSystemUserMessageID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="SystemUserMessageID" required="Yes" type="String">
		
		<cfif SystemUserMessageID NEQ "NULL">	
			
			<cfif arguments.SystemUserMessageID EQ "@@" OR arguments.SystemUserMessageID EQ "">
				
				<cfset arguments.SystemUserMessageID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(SystemUserMessageID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>SystemUserMessageID</strong> (#SystemUserMessageID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.SystemUserMessageID = arguments.SystemUserMessageID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="SystemUserMessageIO" output="No">
		
		<cfargument name="SystemUserMessageID" required="yes" type="numeric">		
		<cfset var qGetSystemUserMessage = "">
		
		<cfset preInit(SystemUserMessageID)>
		...
		<cfscript>
			IF(qGetSystemUserMessage.SystemUserMessageID EQ "" OR qGetSystemUserMessage.SystemUserMessageID EQ "NULL") 
				variables.instance.SystemUserMessageID = "NULL"; 
			ELSE 
				variables.instance.SystemUserMessageID = qGetSystemUserMessage.SystemUserMessageID;
				
			IF(qGetSystemUserMessage.UsersID EQ "" OR qGetSystemUserMessage.UsersID EQ "NULL") 
				variables.instance.UsersID = "NULL"; 
			ELSE 
				variables.instance.UsersID = qGetSystemUserMessage.UsersID;
				
			IF(qGetSystemUserMessage.Dismissed EQ "" OR qGetSystemUserMessage.Dismissed EQ "NULL") 
				variables.instance.Dismissed = "NULL"; 
			ELSE 
				variables.instance.Dismissed = qGetSystemUserMessage.Dismissed;
							
				variables.instance.Note = qGetSystemUserMessage.Note;
							
				variables.instance.Active = qGetSystemUserMessage.Active;
				
			IF(qGetSystemUserMessage.InactiveCode EQ "" OR qGetSystemUserMessage.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetSystemUserMessage.InactiveCode;
							
				variables.instance.DateCreated = qGetSystemUserMessage.DateCreated;
							
				variables.instance.DateModified = qGetSystemUserMessage.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(SystemUserMessageID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitSystemUserMessage = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.SystemUserMessageID) AND variables.instance.SystemUserMessageID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitSystemUserMessage" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitSystemUserMessage" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.SystemUserMessageID = qInsertCommitSystemUserMessage.SystemUserMessageID>
				
				<cfreturn qInsertCommitSystemUserMessage.SystemUserMessageID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "SystemUserMessageID,UsersID,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified">		
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="UsersID,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getSystemUserMessageQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getSystemUserMessageQuery">						
		<cfargument name="Fields" required="No" type="String" default="SystemUserMessageID,UsersID,Dismissed,Note,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="SystemUserMessageID" required="No" type="String" default="">
		<cfargument name="SystemUserMessageID_IN" required="No" type="String" default="">
		<cfargument name="SystemUserMessageID_LIKE" required="No" type="String" default="">
		<cfargument name="UsersID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getSystemUserMessageQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif SystemUserMessageID_LIKE NEQ "">AND SystemUserMessageID LIKE '%#trim(SystemUserMessageID_LIKE)#%'</cfif>
			...
			<cfreturn getSystemUserMessageQuery>
