...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_DIAGNOSISCode table in the dB.           --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_DIAGNOSISCodeTable                                           --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_DIAGNOSISCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_DIAGNOSISCodeIO" output="No">
		...
		<cfquery name="qGetEOB_DIAGNOSISCode" datasource="PA_Master">
		...
		<cfscript>
			IF(qGetEOB_DIAGNOSISCode.RecordID EQ "" OR qGetEOB_DIAGNOSISCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_DIAGNOSISCode.RecordID;
							
				variables.instance.Code = qGetEOB_DIAGNOSISCode.Code;
							
				variables.instance.Description = qGetEOB_DIAGNOSISCode.Description;
							
				variables.instance.Active = qGetEOB_DIAGNOSISCode.Active;
				
			IF(qGetEOB_DIAGNOSISCode.InactiveCode EQ "" OR qGetEOB_DIAGNOSISCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_DIAGNOSISCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_DIAGNOSISCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_DIAGNOSISCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEOB_DIAGNOSISCode = "">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_DIAGNOSISCode" datasource="PA_Master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_DIAGNOSISCode" datasource="PA_Master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_DIAGNOSISCode.RecordID>
				
				<cfreturn qInsertCommitEOB_DIAGNOSISCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EOB_DIAGNOSISCode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO EOB_DIAGNOSISCode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_DIAGNOSISCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_DIAGNOSISCodeQuery">						
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_DIAGNOSISCodeQuery" datasource="PA_Master">
				SELECT #trim(Fields)#
				FROM EOB_DIAGNOSISCode   
				WHERE 1=1
			...
			<cfreturn getEOB_DIAGNOSISCodeQuery>
