...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Definition table in the dB.                  --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_DefinitionTable                                                  --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="DefinitionIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.DefinitionID = "NULL";
			variables.instance.Title = "";
			variables.instance.Definition = "";
...
<!-------------------------------------------------------------------------------------->
	...
	<cfset temp = QueryAddRow(defaultsQuery, 7)>
	
	
<!-------------------------------------------------------------------------------------->
<!--- Set the cells in the query.                                                    --->
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DEFINITIONID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "TITLE", 2)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DEFINITION", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ACTIVE", 4)>
	<cfset temp = QuerySetCell(defaultsQuery, "IS_NULLABLE", "No", 4)>
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "1", 4)>
	<cfset temp = QuerySetCell(defaultsQuery, "DATA_TYPE", "char", 4)>
	
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "INACTIVECODE", 5)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DATECREATED", 6)>
	<cfset temp = QuerySetCell(defaultsQuery, "IS_NULLABLE", "No", 6)>
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_DEFAULT", "NOW()", 6)>
	<cfset temp = QuerySetCell(defaultsQuery, "DATA_TYPE", "datetime", 6)>
	
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DATEMODIFIED", 7)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for DefinitionID.                                           --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getDefinitionID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.DefinitionID>
	</cffunction>
	 
	<cffunction name="setDefinitionID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="DefinitionID" required="Yes" type="String">
		
		<cfif DefinitionID NEQ "NULL">	
			
			<cfif arguments.DefinitionID EQ "@@" OR arguments.DefinitionID EQ "">
				
				<cfset arguments.DefinitionID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(DefinitionID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>DefinitionID</strong> (#DefinitionID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.DefinitionID = arguments.DefinitionID>			
		<cfreturn true>
	...
	<cffunction name="setTitle" access="public" returntype="boolean" output="No">
		...
		<cfif Title NEQ "NULL">	
			...
			<cfelseif len(Title) GT 100>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>Title</strong> cannot be greater than 100 character(s).">
			</cfif>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for Definition.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getDefinition" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.Definition>
	</cffunction>
	 
	<cffunction name="setDefinition" access="public" returntype="boolean" output="No">
 		
		<cfargument name="Definition" required="Yes" type="String">
		
		<cfif Definition NEQ "NULL">	
			
			<cfif arguments.Definition EQ "@@" OR arguments.Definition EQ "">
				
				<cfset arguments.Definition = "NULL">
						
			<cfelseif NOT REQUEST.istextDataType(Definition)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>Definition</strong> (#Definition#) is not of type <strong>text</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.Definition = arguments.Definition>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "Definition")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="Definition" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="DefinitionIO" output="No">
		...
		<cfscript>
			IF(qGetDefinition.DefinitionID EQ "" OR qGetDefinition.DefinitionID EQ "NULL") 
				variables.instance.DefinitionID = "NULL"; 
			ELSE 
				variables.instance.DefinitionID = qGetDefinition.DefinitionID;
							
				variables.instance.Title = qGetDefinition.Title;
							
				variables.instance.Definition = qGetDefinition.Definition;
							
				variables.instance.Active = qGetDefinition.Active;
				
			IF(qGetDefinition.InactiveCode EQ "" OR qGetDefinition.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetDefinition.InactiveCode;
							
				variables.instance.DateCreated = qGetDefinition.DateCreated;
							
				variables.instance.DateModified = qGetDefinition.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(DefinitionID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitDefinition = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.DefinitionID) AND variables.instance.DefinitionID  GT 0>	  		
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.DefinitionID = qInsertCommitDefinition.DefinitionID>
				
				<cfreturn qInsertCommitDefinition.DefinitionID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "DefinitionID,Title,Definition,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "Title,Definition,Active">	
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="Title,Definition,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getDefinitionQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getDefinitionQuery">						
		<cfargument name="Fields" required="No" type="String" default="DefinitionID,Title,Definition,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="DefinitionID" required="No" type="String" default="">
		<cfargument name="DefinitionID_IN" required="No" type="String" default="">
		<cfargument name="DefinitionID_LIKE" required="No" type="String" default="">
		<cfargument name="Title" required="No" type="String" default="">
		<cfargument name="Title_IN" required="No" type="String" default="">
		<cfargument name="Title_LIKE" required="No" type="String" default="">
		<cfargument name="Definition" required="No" type="String" default="">
		<cfargument name="Definition_IN" required="No" type="String" default="">
		<cfargument name="Definition_LIKE" required="No" type="String" default="">
		<cfargument name="Active" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "DefinitionID,DefinitionID_IN,DefinitionID_LIKE,Title,Title_IN,Title_LIKE,Definition,Definition_IN,Definition_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getDefinitionQuery" datasource="PA_Master">
				...
				WHERE 1=1
					...
					<cfif DefinitionID_LIKE NEQ "">AND DefinitionID LIKE '%#trim(DefinitionID_LIKE)#%'</cfif>
					...
					<cfif Definition NEQ "">AND Definition = '#Definition#'</cfif>
					<cfif Definition_IN NEQ "">AND Definition IN(#trim(Definition_IN)#)</cfif>
					<cfif Definition_LIKE NEQ "">AND Definition LIKE '%#trim(Definition_LIKE)#%'</cfif>
