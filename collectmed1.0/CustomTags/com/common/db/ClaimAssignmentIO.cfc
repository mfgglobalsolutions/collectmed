...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the ClaimAssignment table in the dB.             --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_ClaimAssignmentTable                                             --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="ClaimAssignmentIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.ClaimAssignmentID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CLAIMASSIGNMENTID", 1)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for ClaimAssignmentID.                                      --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getClaimAssignmentID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ClaimAssignmentID>
	</cffunction>
	 
	<cffunction name="setClaimAssignmentID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ClaimAssignmentID" required="Yes" type="String">
		
		<cfif ClaimAssignmentID NEQ "NULL">	
			
			<cfif arguments.ClaimAssignmentID EQ "@@" OR arguments.ClaimAssignmentID EQ "">
				
				<cfset arguments.ClaimAssignmentID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(ClaimAssignmentID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ClaimAssignmentID</strong> (#ClaimAssignmentID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.ClaimAssignmentID = arguments.ClaimAssignmentID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="ClaimAssignmentIO" output="No">
		
		<cfargument name="ClaimAssignmentID" required="yes" type="numeric">		
		<cfset var qGetClaimAssignment = "">
		
		<cfset preInit(ClaimAssignmentID)>
		...
		<cfscript>
			IF(qGetClaimAssignment.ClaimAssignmentID EQ "" OR qGetClaimAssignment.ClaimAssignmentID EQ "NULL") 
				variables.instance.ClaimAssignmentID = "NULL"; 
			ELSE 
				variables.instance.ClaimAssignmentID = qGetClaimAssignment.ClaimAssignmentID;
				
			IF(qGetClaimAssignment.ClaimID EQ "" OR qGetClaimAssignment.ClaimID EQ "NULL") 
				variables.instance.ClaimID = "NULL"; 
			ELSE 
				variables.instance.ClaimID = qGetClaimAssignment.ClaimID;
				
			IF(qGetClaimAssignment.UserID EQ "" OR qGetClaimAssignment.UserID EQ "NULL") 
				variables.instance.UserID = "NULL"; 
			ELSE 
				variables.instance.UserID = qGetClaimAssignment.UserID;
							
				variables.instance.DateCreated = qGetClaimAssignment.DateCreated;
				
			IF(qGetClaimAssignment.AssignorID EQ "" OR qGetClaimAssignment.AssignorID EQ "NULL") 
				variables.instance.AssignorID = "NULL"; 
			ELSE 
				variables.instance.AssignorID = qGetClaimAssignment.AssignorID;
							
				variables.instance.Note = qGetClaimAssignment.Note;
				
		</cfscript>		
		  
		<cfset postInit(ClaimAssignmentID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitClaimAssignment = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.ClaimAssignmentID) AND variables.instance.ClaimAssignmentID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitClaimAssignment" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitClaimAssignment" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.ClaimAssignmentID = qInsertCommitClaimAssignment.ClaimAssignmentID>
				
				<cfreturn qInsertCommitClaimAssignment.ClaimAssignmentID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "ClaimAssignmentID,ClaimID,UserID,DateCreated,AssignorID,Note">		
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="ClaimID,UserID,DateCreated,AssignorID,Note" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getClaimAssignmentQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getClaimAssignmentQuery">						
		<cfargument name="Fields" required="No" type="String" default="ClaimAssignmentID,ClaimID,UserID,DateCreated,AssignorID,Note">	
		...
		<cfargument name="ClaimAssignmentID" required="No" type="String" default="">
		<cfargument name="ClaimAssignmentID_IN" required="No" type="String" default="">
		<cfargument name="ClaimAssignmentID_LIKE" required="No" type="String" default="">
		<cfargument name="ClaimID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "ClaimAssignmentID,ClaimAssignmentID_IN,ClaimAssignmentID_LIKE,ClaimID,ClaimID_IN,ClaimID_LIKE,UserID,UserID_IN,UserID_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,AssignorID,AssignorID_IN,AssignorID_LIKE,Note,Note_IN,Note_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getClaimAssignmentQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif ClaimAssignmentID_LIKE NEQ "">AND ClaimAssignmentID LIKE '%#trim(ClaimAssignmentID_LIKE)#%'</cfif>
			...
			<cfreturn getClaimAssignmentQuery>
