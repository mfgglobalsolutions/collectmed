...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_DRGCode table in the dB.                 --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_DRGCodeTable                                                 --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_DRGCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_DRGCodeIO" output="No">
		...
		<cfquery name="qGetEOB_DRGCode" datasource="PA_Master">
		...
		<cfscript>
			IF(qGetEOB_DRGCode.RecordID EQ "" OR qGetEOB_DRGCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_DRGCode.RecordID;
							
				variables.instance.Code = qGetEOB_DRGCode.Code;
							
				variables.instance.Description = qGetEOB_DRGCode.Description;
							
				variables.instance.Active = qGetEOB_DRGCode.Active;
				
			IF(qGetEOB_DRGCode.InactiveCode EQ "" OR qGetEOB_DRGCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_DRGCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_DRGCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_DRGCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_DRGCode" datasource="PA_Master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_DRGCode" datasource="PA_Master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_DRGCode.RecordID>
				
				<cfreturn qInsertCommitEOB_DRGCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EOB_DRGCode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO EOB_DRGCode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_DRGCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_DRGCodeQuery">						
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_DRGCodeQuery" datasource="PA_Master">
				SELECT #trim(Fields)#
				FROM EOB_DRGCode   
				WHERE 1=1
