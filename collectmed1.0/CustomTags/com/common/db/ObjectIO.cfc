...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Object table in the dB.                      --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_ObjectTable                                                      --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "OBJECTID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "OBJECTNAME", 2)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "DESCRIPTION", 3)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for ObjectName.                                             --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getObjectName" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ObjectName>
	</cffunction>
	 
	<cffunction name="setObjectName" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ObjectName" required="Yes" type="String">
		
		<cfif ObjectName NEQ "NULL">	
			
			<cfif arguments.ObjectName EQ "@@" OR arguments.ObjectName EQ "">
				
				<cfset arguments.ObjectName = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(ObjectName)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ObjectName</strong> (#ObjectName#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(ObjectName) GT 50>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>ObjectName</strong> cannot be greater than 50 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.ObjectName = arguments.ObjectName>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "ObjectName")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="ObjectName" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="ObjectIO" output="No">
		...
		<cfscript>
			IF(qGetObject.ObjectID EQ "" OR qGetObject.ObjectID EQ "NULL") 
				variables.instance.ObjectID = "NULL"; 
			ELSE 
				variables.instance.ObjectID = qGetObject.ObjectID;
							
				variables.instance.ObjectName = qGetObject.ObjectName;
							
				variables.instance.Description = qGetObject.Description;
							
				variables.instance.Active = qGetObject.Active;
				
			IF(qGetObject.InactiveCode EQ "" OR qGetObject.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetObject.InactiveCode;
							
				variables.instance.DateCreated = qGetObject.DateCreated;
							
				variables.instance.DateModified = qGetObject.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(ObjectID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitObject = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.ObjectID) AND variables.instance.ObjectID  GT 0>	  		
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.ObjectID = qInsertCommitObject.ObjectID>
				
				<cfreturn qInsertCommitObject.ObjectID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "ObjectID,ObjectName,Description,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "ObjectName,Description,Active">	
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="ObjectName,Description,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getObjectQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getObjectQuery">						
		<cfargument name="Fields" required="No" type="String" default="ObjectID,ObjectName,Description,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="ObjectName" required="No" type="String" default="">
		<cfargument name="ObjectName_IN" required="No" type="String" default="">
		<cfargument name="ObjectName_LIKE" required="No" type="String" default="">
		<cfargument name="Description" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "ObjectID,ObjectID_IN,ObjectID_LIKE,ObjectName,ObjectName_IN,ObjectName_LIKE,Description,Description_IN,Description_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getObjectQuery" datasource="pa_master">
				...
				WHERE 1=1
					...
					<cfif ObjectName NEQ "">AND ObjectName = '#ObjectName#'</cfif>
					<cfif ObjectName_IN NEQ "">AND ObjectName IN(#trim(ObjectName_IN)#)</cfif>
					<cfif ObjectName_LIKE NEQ "">AND ObjectName LIKE '%#trim(ObjectName_LIKE)#%'</cfif>
