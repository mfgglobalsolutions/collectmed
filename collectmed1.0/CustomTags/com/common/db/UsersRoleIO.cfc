...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the UsersRole table in the dB.                   --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_UsersRoleTable                                                   --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="UsersRoleIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.UsersRoleID = "NULL";
			variables.instance.UsersID = "NULL";
			variables.instance.SiteID = "NULL";
			variables.instance.RoleID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "USERSROLEID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ROLEID", 4)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for UsersRoleID.                                            --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getUsersRoleID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.UsersRoleID>
	</cffunction>
	 
	<cffunction name="setUsersRoleID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="UsersRoleID" required="Yes" type="String">
		
		<cfif UsersRoleID NEQ "NULL">	
			
			<cfif arguments.UsersRoleID EQ "@@" OR arguments.UsersRoleID EQ "">
				
				<cfset arguments.UsersRoleID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(UsersRoleID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>UsersRoleID</strong> (#UsersRoleID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.UsersRoleID = arguments.UsersRoleID>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for RoleID.                                                 --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getRoleID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.RoleID>
	</cffunction>
	 
	<cffunction name="setRoleID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="RoleID" required="Yes" type="String">
		
		<cfif RoleID NEQ "NULL">	
			
			<cfif arguments.RoleID EQ "@@" OR arguments.RoleID EQ "">
				
				<cfset arguments.RoleID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(RoleID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>RoleID</strong> (#RoleID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.RoleID = arguments.RoleID>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "RoleID")>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="UsersRoleIO" output="No">
		
		<cfargument name="UsersRoleID" required="yes" type="numeric">		
		<cfset var qGetUsersRole = "">
		
		<cfset preInit(UsersRoleID)>
	
		<cfquery name="qGetUsersRole" datasource="pa_master">
	  		SELECT UsersRoleID,UsersID,SiteID,RoleID,Active,InactiveCode,DateCreated,DateModified
			FROM usersrole  
			WHERE UsersRoleID = #trim(arguments.UsersRoleID)# 
		</cfquery>
		...
		<cfscript>
			IF(qGetUsersRole.UsersRoleID EQ "" OR qGetUsersRole.UsersRoleID EQ "NULL") 
				variables.instance.UsersRoleID = "NULL"; 
			ELSE 
				variables.instance.UsersRoleID = qGetUsersRole.UsersRoleID;
				
			IF(qGetUsersRole.UsersID EQ "" OR qGetUsersRole.UsersID EQ "NULL") 
				variables.instance.UsersID = "NULL"; 
			ELSE 
				variables.instance.UsersID = qGetUsersRole.UsersID;
				
			IF(qGetUsersRole.SiteID EQ "" OR qGetUsersRole.SiteID EQ "NULL") 
				variables.instance.SiteID = "NULL"; 
			ELSE 
				variables.instance.SiteID = qGetUsersRole.SiteID;
				
			IF(qGetUsersRole.RoleID EQ "" OR qGetUsersRole.RoleID EQ "NULL") 
				variables.instance.RoleID = "NULL"; 
			ELSE 
				variables.instance.RoleID = qGetUsersRole.RoleID;
							
				variables.instance.Active = qGetUsersRole.Active;
				
			IF(qGetUsersRole.InactiveCode EQ "" OR qGetUsersRole.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetUsersRole.InactiveCode;
							
				variables.instance.DateCreated = qGetUsersRole.DateCreated;
							
				variables.instance.DateModified = qGetUsersRole.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(UsersRoleID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitUsersRole = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.UsersRoleID) AND variables.instance.UsersRoleID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitUsersRole" datasource="pa_master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitUsersRole" datasource="pa_master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.UsersRoleID = qInsertCommitUsersRole.UsersRoleID>
				
				<cfreturn qInsertCommitUsersRole.UsersRoleID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "UsersRoleID,UsersID,SiteID,RoleID,Active,InactiveCode,DateCreated,DateModified">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE usersrole  SET #columnsToUpdate# WHERE UsersRoleID = #trim(variables.instance.UsersRoleID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="UsersID,SiteID,RoleID,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO usersrole  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS UsersRoleID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getUsersRoleQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getUsersRoleQuery">						
		<cfargument name="Fields" required="No" type="String" default="UsersRoleID,UsersID,SiteID,RoleID,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="UsersRoleID" required="No" type="String" default="">
		<cfargument name="UsersRoleID_IN" required="No" type="String" default="">
		<cfargument name="UsersRoleID_LIKE" required="No" type="String" default="">
		<cfargument name="UsersID" required="No" type="String" default="">
		...
		<cfargument name="RoleID" required="No" type="String" default="">
		<cfargument name="RoleID_IN" required="No" type="String" default="">
		<cfargument name="RoleID_LIKE" required="No" type="String" default="">
		<cfargument name="Active" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "UsersRoleID,UsersRoleID_IN,UsersRoleID_LIKE,UsersID,UsersID_IN,UsersID_LIKE,SiteID,SiteID_IN,SiteID_LIKE,RoleID,RoleID_IN,RoleID_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getUsersRoleQuery" datasource="pa_master">
				SELECT #trim(Fields)#
				FROM usersrole   
				WHERE 1=1
					<cfif UsersRoleID NEQ "" AND IsNumeric(UsersRoleID)>AND UsersRoleID = #UsersRoleID#</cfif>
					<cfif UsersRoleID_IN NEQ "">AND UsersRoleID IN(#trim(UsersRoleID_IN)#)</cfif>
					<cfif UsersRoleID_LIKE NEQ "">AND UsersRoleID LIKE '%#trim(UsersRoleID_LIKE)#%'</cfif>
					...
					<cfif RoleID_LIKE NEQ "">AND RoleID LIKE '%#trim(RoleID_LIKE)#%'</cfif>
