...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the Category table in the dB.                    --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_CategoryTable                                                    --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="CategoryIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.CategoryURL = "";
			variables.instance.CategorySortOrder = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CATEGORYID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CATEGORYNAME", 3)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CATEGORYURL", 4)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CATEGORYSORTORDER", 5)>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getCategoryName" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.CategoryName>
	</cffunction>
	 
	<cffunction name="setCategoryName" access="public" returntype="boolean" output="No">
 		
		<cfargument name="CategoryName" required="Yes" type="String">
		
		<cfif CategoryName NEQ "NULL">	
			
			<cfif arguments.CategoryName EQ "@@" OR arguments.CategoryName EQ "">
				
				<cfset arguments.CategoryName = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(CategoryName)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>CategoryName</strong> (#CategoryName#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(CategoryName) GT 100>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>CategoryName</strong> cannot be greater than 100 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.CategoryName = arguments.CategoryName>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "CategoryName")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for CategoryURL.                                            --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getCategoryURL" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.CategoryURL>
	</cffunction>
	 
	<cffunction name="setCategoryURL" access="public" returntype="boolean" output="No">
 		
		<cfargument name="CategoryURL" required="Yes" type="String">
		
		<cfif CategoryURL NEQ "NULL">	
			
			<cfif arguments.CategoryURL EQ "@@" OR arguments.CategoryURL EQ "">
				
				<cfset arguments.CategoryURL = "NULL">
						
			<cfelseif NOT REQUEST.isvarcharDataType(CategoryURL)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>CategoryURL</strong> (#CategoryURL#) is not of type <strong>varchar</strong>.">	
			<cfelseif len(CategoryURL) GT 100>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>CategoryURL</strong> cannot be greater than 100 character(s).">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.CategoryURL = arguments.CategoryURL>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "CategoryURL")>			
		<cfreturn true>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for CategorySortOrder.                                      --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getCategorySortOrder" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.CategorySortOrder>
	</cffunction>
	 
	<cffunction name="setCategorySortOrder" access="public" returntype="boolean" output="No">
 		
		<cfargument name="CategorySortOrder" required="Yes" type="String">
		
		<cfif CategorySortOrder NEQ "NULL">	
			
			<cfif arguments.CategorySortOrder EQ "@@" OR arguments.CategorySortOrder EQ "">
				
				<cfset arguments.CategorySortOrder = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(CategorySortOrder)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>CategorySortOrder</strong> (#CategorySortOrder#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.CategorySortOrder = arguments.CategorySortOrder>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "CategorySortOrder")>			
		<cfreturn true>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="CategoryName" required="No" type="String" default="">
		<cfargument name="CategoryURL" required="No" type="String" default="">
		<cfargument name="CategorySortOrder" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="CategoryIO" output="No">
		...
		<cfscript>
			IF(qGetCategory.CategoryID EQ "" OR qGetCategory.CategoryID EQ "NULL") 
				variables.instance.CategoryID = "NULL"; 
			ELSE 
				variables.instance.CategoryID = qGetCategory.CategoryID;
				
			IF(qGetCategory.SiteID EQ "" OR qGetCategory.SiteID EQ "NULL") 
				variables.instance.SiteID = "NULL"; 
			ELSE 
				variables.instance.SiteID = qGetCategory.SiteID;
							
				variables.instance.CategoryName = qGetCategory.CategoryName;
							
				variables.instance.CategoryURL = qGetCategory.CategoryURL;
				
			IF(qGetCategory.CategorySortOrder EQ "" OR qGetCategory.CategorySortOrder EQ "NULL") 
				variables.instance.CategorySortOrder = "NULL"; 
			ELSE 
				variables.instance.CategorySortOrder = qGetCategory.CategorySortOrder;
							
				variables.instance.Active = qGetCategory.Active;
				
			IF(qGetCategory.InactiveCode EQ "" OR qGetCategory.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetCategory.InactiveCode;
							
				variables.instance.DateCreated = qGetCategory.DateCreated;
							
				variables.instance.DateModified = qGetCategory.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(CategoryID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitCategory = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.CategoryID) AND variables.instance.CategoryID  GT 0>	  		
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.CategoryID = qInsertCommitCategory.CategoryID>
				
				<cfreturn qInsertCommitCategory.CategoryID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "CategoryID,SiteID,CategoryName,CategoryURL,CategorySortOrder,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "CategoryName,CategoryURL,Active">	
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="SiteID,CategoryName,CategoryURL,CategorySortOrder,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getCategoryQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getCategoryQuery">						
		<cfargument name="Fields" required="No" type="String" default="CategoryID,SiteID,CategoryName,CategoryURL,CategorySortOrder,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="CategoryName" required="No" type="String" default="">
		<cfargument name="CategoryName_IN" required="No" type="String" default="">
		<cfargument name="CategoryName_LIKE" required="No" type="String" default="">
		<cfargument name="CategoryURL" required="No" type="String" default="">
		<cfargument name="CategoryURL_IN" required="No" type="String" default="">
		<cfargument name="CategoryURL_LIKE" required="No" type="String" default="">
		<cfargument name="CategorySortOrder" required="No" type="String" default="">
		<cfargument name="CategorySortOrder_IN" required="No" type="String" default="">
		<cfargument name="CategorySortOrder_LIKE" required="No" type="String" default="">
		<cfargument name="Active" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "CategoryID,CategoryID_IN,CategoryID_LIKE,SiteID,SiteID_IN,SiteID_LIKE,CategoryName,CategoryName_IN,CategoryName_LIKE,CategoryURL,CategoryURL_IN,CategoryURL_LIKE,CategorySortOrder,CategorySortOrder_IN,CategorySortOrder_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getCategoryQuery" datasource="pa_master">
				...
				WHERE 1=1
					...
					<cfif CategoryName NEQ "">AND CategoryName = '#CategoryName#'</cfif>
					<cfif CategoryName_IN NEQ "">AND CategoryName IN(#trim(CategoryName_IN)#)</cfif>
					<cfif CategoryName_LIKE NEQ "">AND CategoryName LIKE '%#trim(CategoryName_LIKE)#%'</cfif>
					<cfif CategoryURL NEQ "">AND CategoryURL = '#CategoryURL#'</cfif>
					<cfif CategoryURL_IN NEQ "">AND CategoryURL IN(#trim(CategoryURL_IN)#)</cfif>
					<cfif CategoryURL_LIKE NEQ "">AND CategoryURL LIKE '%#trim(CategoryURL_LIKE)#%'</cfif>
					<cfif CategorySortOrder NEQ "" AND IsNumeric(CategorySortOrder)>AND CategorySortOrder = #CategorySortOrder#</cfif>
					<cfif CategorySortOrder_IN NEQ "">AND CategorySortOrder IN(#trim(CategorySortOrder_IN)#)</cfif>
					<cfif CategorySortOrder_LIKE NEQ "">AND CategorySortOrder LIKE '%#trim(CategorySortOrder_LIKE)#%'</cfif>
