...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_DMEFEETOSCode table in the dB.           --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_DMEFEETOSCodeTable                                           --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_DMEFEETOSCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_DMEFEETOSCodeIO" output="No">
		...
		<cfquery name="qGetEOB_DMEFEETOSCode" datasource="pa_master">
		...
		<cfscript>
			IF(qGetEOB_DMEFEETOSCode.RecordID EQ "" OR qGetEOB_DMEFEETOSCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_DMEFEETOSCode.RecordID;
							
				variables.instance.Code = qGetEOB_DMEFEETOSCode.Code;
							
				variables.instance.Description = qGetEOB_DMEFEETOSCode.Description;
							
				variables.instance.Active = qGetEOB_DMEFEETOSCode.Active;
				
			IF(qGetEOB_DMEFEETOSCode.InactiveCode EQ "" OR qGetEOB_DMEFEETOSCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_DMEFEETOSCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_DMEFEETOSCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_DMEFEETOSCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEOB_DMEFEETOSCode = "">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_DMEFEETOSCode" datasource="pa_master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_DMEFEETOSCode" datasource="pa_master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_DMEFEETOSCode.RecordID>
				
				<cfreturn qInsertCommitEOB_DMEFEETOSCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE eob_dmefeetoscode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO eob_dmefeetoscode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_DMEFEETOSCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_DMEFEETOSCodeQuery">						
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_DMEFEETOSCodeQuery" datasource="pa_master">
				SELECT #trim(Fields)#
				FROM eob_dmefeetoscode   
				WHERE 1=1
			...
			<cfreturn getEOB_DMEFEETOSCodeQuery>
