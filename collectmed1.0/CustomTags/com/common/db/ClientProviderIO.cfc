...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the ClientProvider table in the dB.              --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_ClientProviderTable                                              --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="ClientProviderIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.ClientProviderID = "NULL";
...
<!-------------------------------------------------------------------------------------->
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "CLIENTPROVIDERID", 1)>
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "PROVIDERID", 3)>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for ClientProviderID.                                       --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getClientProviderID" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.ClientProviderID>
	</cffunction>
	 
	<cffunction name="setClientProviderID" access="public" returntype="boolean" output="No">
 		
		<cfargument name="ClientProviderID" required="Yes" type="String">
		
		<cfif ClientProviderID NEQ "NULL">	
			
			<cfif arguments.ClientProviderID EQ "@@" OR arguments.ClientProviderID EQ "">
				
				<cfset arguments.ClientProviderID = "NULL">
						
			<cfelseif NOT REQUEST.isintDataType(ClientProviderID)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>ClientProviderID</strong> (#ClientProviderID#) is not of type <strong>int</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.ClientProviderID = arguments.ClientProviderID>			
		<cfreturn true>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="ClientProviderIO" output="No">
		
		<cfargument name="ClientProviderID" required="yes" type="numeric">		
		<cfset var qGetClientProvider = "">
		
		<cfset preInit(ClientProviderID)>
		...
		<cfscript>
			IF(qGetClientProvider.ClientProviderID EQ "" OR qGetClientProvider.ClientProviderID EQ "NULL") 
				variables.instance.ClientProviderID = "NULL"; 
			ELSE 
				variables.instance.ClientProviderID = qGetClientProvider.ClientProviderID;
				
			IF(qGetClientProvider.ClientID EQ "" OR qGetClientProvider.ClientID EQ "NULL") 
				variables.instance.ClientID = "NULL"; 
			ELSE 
				variables.instance.ClientID = qGetClientProvider.ClientID;
							
				variables.instance.ProviderID = qGetClientProvider.ProviderID;
							
				variables.instance.Active = qGetClientProvider.Active;
				
			IF(qGetClientProvider.InactiveCode EQ "" OR qGetClientProvider.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetClientProvider.InactiveCode;
							
				variables.instance.DateCreated = qGetClientProvider.DateCreated;
							
				variables.instance.DateModified = qGetClientProvider.DateModified;
				
		</cfscript>		
		  
		<cfset postInit(ClientProviderID)>
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitClientProvider = "">
		
		<cfset preUpdateCommit()>
	 	
		<cfif IsNumeric(variables.instance.ClientProviderID) AND variables.instance.ClientProviderID  GT 0>	  		
			
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
			
				<cfset sqlStatement = preUpdateDataCheck()>				
				
				<cfquery name="qUpdateCommitClientProvider" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitClientProvider" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.ClientProviderID = qInsertCommitClientProvider.ClientProviderID>
				
				<cfreturn qInsertCommitClientProvider.ClientProviderID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "ClientProviderID,ClientID,ProviderID,Active,InactiveCode,DateCreated,DateModified">		
			
		<!-------------------------------------------------------------------------------------->
		<!--- Denote the column names that need single quotes (strings).                     --->
		<!-------------------------------------------------------------------------------------->
			<cfset stringTableColumns = "ProviderID,Active">	
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="ClientID,ProviderID,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getClientProviderQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getClientProviderQuery">						
		<cfargument name="Fields" required="No" type="String" default="ClientProviderID,ClientID,ProviderID,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="ClientProviderID" required="No" type="String" default="">
		<cfargument name="ClientProviderID_IN" required="No" type="String" default="">
		<cfargument name="ClientProviderID_LIKE" required="No" type="String" default="">
		<cfargument name="ClientID" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "ClientProviderID,ClientProviderID_IN,ClientProviderID_LIKE,ClientID,ClientID_IN,ClientID_LIKE,ProviderID,ProviderID_IN,ProviderID_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getClientProviderQuery" datasource="#trim(request.datasource)#">
				...
				WHERE 1=1
					...
					<cfif ClientProviderID_LIKE NEQ "">AND ClientProviderID LIKE '%#trim(ClientProviderID_LIKE)#%'</cfif>
			...
			<cfreturn getClientProviderQuery>
