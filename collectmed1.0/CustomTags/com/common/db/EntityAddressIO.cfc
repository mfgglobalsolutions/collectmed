...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EntityAddress table in the dB.               --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EntityAddressTable                                               --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EntityAddressIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ADDRESSID", 3)>
	...
	<cffunction name="setAddressID" access="public" returntype="boolean" output="No">
		...
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "AddressID")>			
		<cfreturn true>
	...
	<cffunction name="Init" access="public" returntype="EntityAddressIO" output="No">
		...
		<cfscript>
			IF(qGetEntityAddress.recordID EQ "" OR qGetEntityAddress.recordID EQ "NULL") 
				variables.instance.recordID = "NULL"; 
			ELSE 
				variables.instance.recordID = qGetEntityAddress.recordID;
				
			IF(qGetEntityAddress.EntityID EQ "" OR qGetEntityAddress.EntityID EQ "NULL") 
				variables.instance.EntityID = "NULL"; 
			ELSE 
				variables.instance.EntityID = qGetEntityAddress.EntityID;
				
			IF(qGetEntityAddress.AddressID EQ "" OR qGetEntityAddress.AddressID EQ "NULL") 
				variables.instance.AddressID = "NULL"; 
			ELSE 
				variables.instance.AddressID = qGetEntityAddress.AddressID;
							
				variables.instance.IsDefault = qGetEntityAddress.IsDefault;
							
				variables.instance.Active = qGetEntityAddress.Active;
				
			IF(qGetEntityAddress.InactiveCode EQ "" OR qGetEntityAddress.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEntityAddress.InactiveCode;
							
				variables.instance.DateCreated = qGetEntityAddress.DateCreated;
							
				variables.instance.DateModified = qGetEntityAddress.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEntityAddress = "">
		...
		<cfif IsNumeric(variables.instance.recordID) AND variables.instance.recordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEntityAddress" datasource="#trim(request.datasource)#">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEntityAddress" datasource="#trim(request.datasource)#">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.recordID = qInsertCommitEntityAddress.recordID>
				
				<cfreturn qInsertCommitEntityAddress.recordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "recordID,EntityID,AddressID,IsDefault,Active,InactiveCode,DateCreated,DateModified">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE entityaddress  SET #columnsToUpdate# WHERE recordID = #trim(variables.instance.recordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="EntityID,AddressID,IsDefault,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEntityAddressQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEntityAddressQuery">						
		<cfargument name="Fields" required="No" type="String" default="recordID,EntityID,AddressID,IsDefault,Active,InactiveCode,DateCreated,DateModified">	
		...
		<!-------------------------------------------------------------------------------------->			
			<cfset availableColumns = "recordID,recordID_IN,recordID_LIKE,EntityID,EntityID_IN,EntityID_LIKE,AddressID,AddressID_IN,AddressID_LIKE,IsDefault,IsDefault_IN,IsDefault_LIKE,Active,Active_IN,Active_LIKE,InactiveCode,InactiveCode_IN,InactiveCode_LIKE,DateCreated,DateCreated_IN,DateCreated_LIKE,DateModified,DateModified_IN,DateModified_LIKE,top1">
		...
		<!-------------------------------------------------------------------------------------->			
			...
			<cfreturn getEntityAddressQuery>
