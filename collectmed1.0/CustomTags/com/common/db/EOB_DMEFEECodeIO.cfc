...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_DMEFEECode table in the dB.              --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_DMEFEECodeTable                                              --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_DMEFEECodeIO">
	...
	<cffunction name="reset" access="public" output="no">
		...
		<cfscript>
			...
			variables.instance.AccessBasedMaxFee = "NULL";
...
<!-------------------------------------------------------------------------------------->
	...
	<cfset temp = QuerySetCell(defaultsQuery, "COLUMN_NAME", "ACCESSBASEDMAXFEE", 8)>
	...
	<cffunction name="setTOSCode" access="public" returntype="boolean" output="No">
		...
		<cfif TOSCode NEQ "NULL">	
			...
			<cfelseif len(TOSCode) GT 3>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>TOSCode</strong> cannot be greater than 3 character(s).">
			</cfif>
	...
	<!--------------------------------------------------------------------------------------> 	
	<!--- GET and SET method for AccessBasedMaxFee.                                      --->
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getAccessBasedMaxFee" access="public" returntype="String" output="No">
  		<cfreturn variables.instance.AccessBasedMaxFee>
	</cffunction>
	 
	<cffunction name="setAccessBasedMaxFee" access="public" returntype="boolean" output="No">
 		
		<cfargument name="AccessBasedMaxFee" required="Yes" type="String">
		
		<cfif AccessBasedMaxFee NEQ "NULL">	
			
			<cfif arguments.AccessBasedMaxFee EQ "@@" OR arguments.AccessBasedMaxFee EQ "">
				
				<cfset arguments.AccessBasedMaxFee = "NULL">
						
			<cfelseif NOT REQUEST.isfloatDataType(AccessBasedMaxFee)>
				<cf_gcGeneralErrorTemplate				
					message="<strong>AccessBasedMaxFee</strong> (#AccessBasedMaxFee#) is not of type <strong>float</strong>.">
			</cfif>
						
		</cfif>	
			
 		<cfset variables.instance.AccessBasedMaxFee = arguments.AccessBasedMaxFee>					
		<cfset variables.instance.changedVariables = ListAppend(variables.instance.changedVariables, "AccessBasedMaxFee")>			
		<cfreturn true>
	...
	<cffunction name="setNoteCode" access="public" returntype="boolean" output="No">
		...
		<cfif NoteCode NEQ "NULL">	
			...
			<cfelseif len(NoteCode) GT 30>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>NoteCode</strong> cannot be greater than 30 character(s).">
			</cfif>
	...
	<cffunction name="new" access="public" output="no">
		...
		<cfargument name="AccessBasedMaxFee" required="No" type="String" default="">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_DMEFEECodeIO" output="No">
		...
		<cfscript>
			IF(qGetEOB_DMEFEECode.RecordID EQ "" OR qGetEOB_DMEFEECode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_DMEFEECode.RecordID;
							
				variables.instance.TOSCode = qGetEOB_DMEFEECode.TOSCode;
							
				variables.instance.Code = qGetEOB_DMEFEECode.Code;
							
				variables.instance.Description = qGetEOB_DMEFEECode.Description;
				
			IF(qGetEOB_DMEFEECode.TMRMpayable EQ "" OR qGetEOB_DMEFEECode.TMRMpayable EQ "NULL") 
				variables.instance.TMRMpayable = "NULL"; 
			ELSE 
				variables.instance.TMRMpayable = qGetEOB_DMEFEECode.TMRMpayable;
				
			IF(qGetEOB_DMEFEECode.TotalRVUs EQ "" OR qGetEOB_DMEFEECode.TotalRVUs EQ "NULL") 
				variables.instance.TotalRVUs = "NULL"; 
			ELSE 
				variables.instance.TotalRVUs = qGetEOB_DMEFEECode.TotalRVUs;
				
			IF(qGetEOB_DMEFEECode.ConversionFactor EQ "" OR qGetEOB_DMEFEECode.ConversionFactor EQ "NULL") 
				variables.instance.ConversionFactor = "NULL"; 
			ELSE 
				variables.instance.ConversionFactor = qGetEOB_DMEFEECode.ConversionFactor;
				
			IF(qGetEOB_DMEFEECode.AccessBasedMaxFee EQ "" OR qGetEOB_DMEFEECode.AccessBasedMaxFee EQ "NULL") 
				variables.instance.AccessBasedMaxFee = "NULL"; 
			ELSE 
				variables.instance.AccessBasedMaxFee = qGetEOB_DMEFEECode.AccessBasedMaxFee;
							
				variables.instance.NoteCode = qGetEOB_DMEFEECode.NoteCode;
							
				variables.instance.Active = qGetEOB_DMEFEECode.Active;
				
			IF(qGetEOB_DMEFEECode.InactiveCode EQ "" OR qGetEOB_DMEFEECode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_DMEFEECode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_DMEFEECode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_DMEFEECode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEOB_DMEFEECode = "">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_DMEFEECode" datasource="PA_Master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_DMEFEECode" datasource="PA_Master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_DMEFEECode.RecordID>
				
				<cfreturn qInsertCommitEOB_DMEFEECode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->	
			<cfset tableColumns = "RecordID,TOSCode,Code,Description,TMRMpayable,TotalRVUs,ConversionFactor,AccessBasedMaxFee,NoteCode,Active,InactiveCode,DateCreated,DateModified">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EOB_DMEFEECode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!--- append it to the update statement.                                             --->
		<!-------------------------------------------------------------------------------------->					
			<cfloop list="TOSCode,Code,Description,TMRMpayable,TotalRVUs,ConversionFactor,AccessBasedMaxFee,NoteCode,Active,InactiveCode,DateCreated,DateModified" index="currentVar">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_DMEFEECodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_DMEFEECodeQuery">						
		<cfargument name="Fields" required="No" type="String" default="RecordID,TOSCode,Code,Description,TMRMpayable,TotalRVUs,ConversionFactor,AccessBasedMaxFee,NoteCode,Active,InactiveCode,DateCreated,DateModified">	
		...
		<cfargument name="AccessBasedMaxFee" required="No" type="String" default="">
		<cfargument name="AccessBasedMaxFee_IN" required="No" type="String" default="">
		<cfargument name="AccessBasedMaxFee_LIKE" required="No" type="String" default="">
		<cfargument name="NoteCode" required="No" type="String" default="">
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_DMEFEECodeQuery" datasource="PA_Master">
				...
				WHERE 1=1
					...
					<cfif AccessBasedMaxFee NEQ "" AND IsNumeric(AccessBasedMaxFee)>AND AccessBasedMaxFee = #AccessBasedMaxFee#</cfif>
					<cfif AccessBasedMaxFee_IN NEQ "">AND AccessBasedMaxFee IN(#trim(AccessBasedMaxFee_IN)#)</cfif>
					<cfif AccessBasedMaxFee_LIKE NEQ "">AND AccessBasedMaxFee LIKE '%#trim(AccessBasedMaxFee_LIKE)#%'</cfif>
			...
			<cfreturn getEOB_DMEFEECodeQuery>
