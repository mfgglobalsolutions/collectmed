...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_EOPSCode table in the dB.                --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_EOPSCodeTable                                                --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_EOPSCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_EOPSCodeIO" output="No">
		
		<cfargument name="RecordID" required="yes" type="numeric">		
		<cfset var qGetEOB_EOPSCode = "">
		...
		<cfif qGetEOB_EOPSCode.Recordcount LTE 0>
			<cf_gcGeneralErrorTemplate				
		...
		<cfscript>
			IF(qGetEOB_EOPSCode.RecordID EQ "" OR qGetEOB_EOPSCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_EOPSCode.RecordID;
							
				variables.instance.Code = qGetEOB_EOPSCode.Code;
							
				variables.instance.Description = qGetEOB_EOPSCode.Description;
							
				variables.instance.Active = qGetEOB_EOPSCode.Active;
				
			IF(qGetEOB_EOPSCode.InactiveCode EQ "" OR qGetEOB_EOPSCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_EOPSCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_EOPSCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_EOPSCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.RecordID = qInsertCommitEOB_EOPSCode.RecordID>
				
				<cfreturn qInsertCommitEOB_EOPSCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EOB_EOPSCode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO EOB_EOPSCode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_EOPSCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_EOPSCodeQuery">						
