...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_MEDICARE_DXCode table in the dB.         --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_MEDICARE_DXCodeTable                                         --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_MEDICARE_DXCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<cffunction name="setCode" access="public" returntype="boolean" output="No">
		...
		<cfif Code NEQ "NULL">	
			...
			<cfelseif len(Code) GT 10>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>Code</strong> cannot be greater than 10 character(s).">
			</cfif>
	...
	<cffunction name="setDescription" access="public" returntype="boolean" output="No">
		...
		<cfif Description NEQ "NULL">	
			...
			<cfelseif len(Description) GT 200>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>Description</strong> cannot be greater than 200 character(s).">
			</cfif>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_MEDICARE_DXCodeIO" output="No">
		...
		<cfscript>
			IF(qGetEOB_MEDICARE_DXCode.RecordID EQ "" OR qGetEOB_MEDICARE_DXCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_MEDICARE_DXCode.RecordID;
							
				variables.instance.Code = qGetEOB_MEDICARE_DXCode.Code;
							
				variables.instance.Description = qGetEOB_MEDICARE_DXCode.Description;
							
				variables.instance.Active = qGetEOB_MEDICARE_DXCode.Active;
				
			IF(qGetEOB_MEDICARE_DXCode.InactiveCode EQ "" OR qGetEOB_MEDICARE_DXCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_MEDICARE_DXCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_MEDICARE_DXCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_MEDICARE_DXCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
	 	...
	 	<cfset var qCommitEOB_MEDICARE_DXCode = "">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.RecordID = qInsertCommitEOB_MEDICARE_DXCode.RecordID>
				
				<cfreturn qInsertCommitEOB_MEDICARE_DXCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE EOB_MEDICARE_DXCode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO EOB_MEDICARE_DXCode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_MEDICARE_DXCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_MEDICARE_DXCodeQuery">						
		...
		<!-------------------------------------------------------------------------------------->			
			...
			<cfreturn getEOB_MEDICARE_DXCodeQuery>
