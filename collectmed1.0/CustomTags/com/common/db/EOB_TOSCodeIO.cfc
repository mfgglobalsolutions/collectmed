...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_TOSCode table in the dB.                 --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_TOSCodeTable                                                 --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_TOSCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<cffunction name="setDescription" access="public" returntype="boolean" output="No">
		...
		<cfif Description NEQ "NULL">	
			...
			<cfelseif len(Description) GT 8000>
				<cf_gcGeneralErrorTemplate
					message="The field <strong>Description</strong> cannot be greater than 8000 character(s).">
			</cfif>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_TOSCodeIO" output="No">
		...
		<cfscript>
			IF(qGetEOB_TOSCode.RecordID EQ "" OR qGetEOB_TOSCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_TOSCode.RecordID;
							
				variables.instance.Code = qGetEOB_TOSCode.Code;
							
				variables.instance.Description = qGetEOB_TOSCode.Description;
							
				variables.instance.Active = qGetEOB_TOSCode.Active;
				
			IF(qGetEOB_TOSCode.InactiveCode EQ "" OR qGetEOB_TOSCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_TOSCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_TOSCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_TOSCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfset variables.instance.RecordID = qInsertCommitEOB_TOSCode.RecordID>
				
				<cfreturn qInsertCommitEOB_TOSCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE eob_toscode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_TOSCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_TOSCodeQuery">						
