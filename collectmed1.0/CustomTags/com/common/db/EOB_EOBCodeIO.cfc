...
<!--- DESCRIPTION:                                                                   ---> 
<!--- @@DESCRIPTION@@                                                                --->
<!--- This tag is a CFC tag tied to the EOB_EOBCode table in the dB.                 --->  
<!--- @@DESCRIPTION@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- EXAMPLE CALL:                                                                  --->
<!--- @@ExampleCall@@                                                                --->
<!--- &lt;cf_db_Get_EOB_EOBCodeTable                                                 --->  
<!--- @@ExampleCall@@                                                                --->
<!---                                                                                --->
<!-------------------------------------------------------------------------------------->
<!--- REQUIRED PARAMETERS:                                                           --->
...
<cfcomponent name="EOB_EOBCodeIO">
...
<!-------------------------------------------------------------------------------------->
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="Init" access="public" returntype="EOB_EOBCodeIO" output="No">
		...
		<cfquery name="qGetEOB_EOBCode" datasource="pa_master">
		...
		<cfscript>
			IF(qGetEOB_EOBCode.RecordID EQ "" OR qGetEOB_EOBCode.RecordID EQ "NULL") 
				variables.instance.RecordID = "NULL"; 
			ELSE 
				variables.instance.RecordID = qGetEOB_EOBCode.RecordID;
							
				variables.instance.Code = qGetEOB_EOBCode.Code;
							
				variables.instance.Description = qGetEOB_EOBCode.Description;
							
				variables.instance.Active = qGetEOB_EOBCode.Active;
				
			IF(qGetEOB_EOBCode.InactiveCode EQ "" OR qGetEOB_EOBCode.InactiveCode EQ "NULL") 
				variables.instance.InactiveCode = "NULL"; 
			ELSE 
				variables.instance.InactiveCode = qGetEOB_EOBCode.InactiveCode;
							
				variables.instance.DateCreated = qGetEOB_EOBCode.DateCreated;
							
				variables.instance.DateModified = qGetEOB_EOBCode.DateModified;
	...
	<cffunction name="Commit" access="public" returntype="Boolean">
		...
		<cfif IsNumeric(variables.instance.RecordID) AND variables.instance.RecordID  GT 0>	  		
			...
			<cfif ListLen(trim(variables.instance.changedVariables)) GTE 1>
				...
				<cfquery name="qUpdateCommitEOB_EOBCode" datasource="pa_master">
		...
		<cfelse>
			...
			<cfelse>
				...
				<cfquery name="qInsertCommitEOB_EOBCode" datasource="pa_master">
		   			#PreserveSingleQuotes(sqlStatement)#
		   		</cfquery>		
		 					
				<cfset postInsertCommit()>
				
				<cfset variables.instance.RecordID = qInsertCommitEOB_EOBCode.RecordID>
				
				<cfreturn qInsertCommitEOB_EOBCode.RecordID>
	...
	<cffunction name="preUpdateDataCheck" output="no">
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "UPDATE eob_eobcode  SET #columnsToUpdate# WHERE RecordID = #trim(variables.instance.RecordID)#">
	...
	<cffunction name="preInsertDataCheck" output="no">		
		...
		<!-------------------------------------------------------------------------------------->					
			...
			<!--- Build the SQL statement.                                                       --->
			<!-------------------------------------------------------------------------------------->	
				<cfset sqlStatement = "INSERT INTO eob_eobcode  (#trim(columnsToInsert)#) VALUES(#trim(columnsToInsertValues)#); SELECT LAST_INSERT_ID() AS RecordID ">
	...
	<!--- query.                                                                         --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getEOB_EOBCodeQuery" returntype="query" hint="Call this object by appending to the where clause the columns that match your search criteria returning a query.">
		
		<cfargument name="QueryName" required="No" type="String" default="getEOB_EOBCodeQuery">						
		...
		<!-------------------------------------------------------------------------------------->			
			<cfquery Name="getEOB_EOBCodeQuery" datasource="pa_master">
				SELECT #trim(Fields)#
				FROM eob_eobcode   
				WHERE 1=1
