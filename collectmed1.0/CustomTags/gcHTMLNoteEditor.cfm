...
<!------------------------------------------------------------------>		
	...
	<cfparam name="attributes.editHeight" default="100">
	<cfset editHeight = attributes.editHeight>
	
	<cfparam name="attributes.editWidthPercent" default="100">
	<cfset editWidthPercent = attributes.editWidthPercent>
	
	<cfparam name="attributes.Identifier" default="1">
...
<cfif ThisTag.executionMode EQ "End" OR (NOT ThisTag.HasEndTag)>
	...
	<cfscript>		
		thisEditor = #trim(Identifier)#;
		if (NOT IsDefined("Attributes.width")) Attributes.width = "#trim(editWidthPercent)#%";
		if (NOT IsDefined("Attributes.height")) Attributes.height = "#trim(editHeight)#";
	...
	<cfif IsDefined("verbage")>
		<cfset verbage = ReplaceNoCase(trim(verbage), CHR(34), "", "ALL")>
		<cfset verbage = ReplaceNoCase(trim(verbage), CHR(10), "", "ALL")>
		<cfset verbage = ReplaceNoCase(trim(verbage), CHR(13), "", "ALL")>
	</cfif>		
	...
	<script language="JavaScript">
		...
		function colorPicker<cfoutput>#trim(Identifier)#</cfoutput>(ed,type) {
			var DHTMLEditor = document.frames("DHTMLEditor"+ed);
			DHTMLEditor.focus();
			if (isIE6 && document.getElementById("dlgHelper<cfoutput>#trim(Identifier)#</cfoutput>")) {
				var dlgHelper<cfoutput>#trim(Identifier)#</cfoutput> = document.getElementById("dlgHelper<cfoutput>#trim(Identifier)#</cfoutput>");
				thisColor = dlgHelper<cfoutput>#trim(Identifier)#</cfoutput>.ChooseColorDlg();
		...
		function spellCheck<cfoutput>#trim(Identifier)#</cfoutput>(ed){
			api=window.open('appSpellCheck.cfm?identifier=<cfoutput>#trim(Identifier)#</cfoutput>' , 'api', 'top=150,left=150,dependent=yes,directories=no,hotkeys=no,location=no,copyhistory=no,scrollbars=no,toolbar=no,menubar=no,resizable=yes,width=290,height=175'); api.focus(0);
		};	
	...
	<OBJECT id=dlgHelper<cfoutput>#trim(Identifier)#</cfoutput> CLASSID="clsid:3050f819-98b5-11cf-bb82-00aa00bdce0b" width="0px" height="0px"></OBJECT>
	<cfhtmlhead text="
	<style>
	...
	<cfoutput>
		...
		<div id="DHTMLEditorToolbar#thisEditor#" align="left" style="background-color:buttonface;width:#trim(editWidthPercent)#%;height:18px;">					
			...
			--->
			<img src="../images/WebAuthorImages/button_spellcheck.gif" style="cursor: 'hand';" alt="Spell Check" onclick="spellCheck<cfoutput>#trim(Identifier)#</cfoutput>(#thisEditor#);" border="0">
			...
			---><img src="../images/WebAuthorImages/separator.gif" alt="" border="0">		
			<img src="../images/WebAuthorImages/button_font_color.gif" style="cursor: 'hand';" alt="Font Color" onclick="colorPicker<cfoutput>#trim(Identifier)#</cfoutput>(#thisEditor#,'ForeColor');" border="0">
			<img src="../images/WebAuthorImages/button_bgcolor.gif" style="cursor: 'hand';" alt="Background Color" onclick="colorPicker<cfoutput>#trim(Identifier)#</cfoutput>(#thisEditor#,'BackColor');" border="0">
		</div>

		<div id="Wrapper">			
			<iFrame src="blank.html" frameborder="1" contenteditable HEIGHT=90% id="DHTMLEditor#thisEditor#" style="border:0;width:#Attributes.width#;height:#Attributes.height#;"></iFrame>
		...
		<cfif CGI.REMOTE_ADDR EQ "192.168.1.11">
		...
		<script language="JavaScript">
			...
			function beforeSubmit(){			
				//alert("document.all.#attributes.field_Name#.value = document.frames('DHTMLEditor#thisEditor#').document.body.innerHTML;");
