		...
		<cftry>			
			...
			<cfoutput>
				<script language="JavaScript">
					var taskElement = document.getElementById("tabQuoteClosedVerbage");
					if (taskElement != null){
						document.getElementById("tabQuoteClosedVerbage").innerHTML = 'Closed (#trim(getQuotes.RecordCount)#)';		
					};	
				</script>
			</cfoutput>					
					...
					<cfloop query="getQuotes" startRow="#trim(startRow_Quote)#" endRow="#trim(endRow_Quote)#">					
						...
						<cfoutput>							
							<tr bgcolor=#trim(rowColor)# style="cursor:hand" onmouseover="this.style.backgroundColor='lightgrey';" onmouseout="this.style.backgroundColor='#trim(rowColor)#';" onclick="location.href='appQuoteEdit.cfm?quoteID=#trim(quoteID)#'">										
								...
								<td>&nbsp;</td>	
								<td class="SiteLabel" nowrap>#DateFormat(CreateDate(trim(patientDOBYY), trim(patientDOBMM), trim(patientDOBDD)), "mm/dd/yyyy")#</td>
								<td>&nbsp;</td>	
