...
<!-------------------------------------------------------------------------------------->
	<cfset PhoneObj = createObject("component", "phoneGateway")>
	<cfset phone = PhoneObj.getByID(ds: trim(request.datasource), phoneID: trim(phoneID))>	
								...
								<li>
									...
									<select class="SiteSelectBox" name="PhoneTypeID">											
										<cfloop query="getAllPhoneTypes">
											<option value="#StandardListItemID#" <cfif StandardListItemID EQ phone.getPhoneTypeID()>SELECTED</cfif>> #ItemNameDisplay#
										</cfloop>
									</select>
								</li>
								<li>
									<label for="Attention">Phone Number:</label>
									<input class="SiteTextbox" name="PhoneNumber" type="text" size="18" value="#request.formatPhoneHyphen(trim(application.beanFactory.getBean('globalFooter').GlobalFooterD(phone.getPhoneNumber())))#">
									ext. <input class="SiteTextbox" name="PhoneExtension" type="text" size="7" value="#trim(application.beanFactory.getBean('globalFooter').GlobalFooterD(phone.getPhoneExtension()))#">
								</li>	
								<li>
