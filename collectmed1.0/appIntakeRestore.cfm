...
<!------------------------------------------------------------------>
	...
	<cfparam name="attributes.formname" default="restoreIntake">
		...
		<script language="JavaScript">
			...
			function validateForm(){
				...
				if(document.#formName#.note.value == "" || document.#formName#.note.value.length <= 2){
					msg = msg + "\n\n Please enter a note (minimum 5 characters) on why this intake is being restored.";
				};
...
<cfif NOT IsDefined("form.Submitted")>
	...
	<cfoutput>
		...
		<div align="left">
			...
			<cf_gcBorderedTable	title="Restore Intake" tablealign="left">
					...
					<table border="0" width="100%" cellpadding="0" cellspacing="10">
						...
						<tr>
							<td class="#trim(class)#" valign="top">
					   			Reason for Restoring Intake: #trim(intakeID)#
							</td>
						</tr>
						<tr>
							<td>
						...
						<tr>
							<td align="center">
								<cf_gcSubmitButton value="Restore & Close">&nbsp;&nbsp;&nbsp;<input type="Button" class="SiteSubmitbox" value="Cancel" onclick="self.close();">
							</td>
						</tr>
...
<cfelseif IsDefined("form.Submitted")>
	...
	<cfif IsDefined("intakeID") AND IsNumeric(intakeID)>
		...
		<cfif ListContains(session.User.getRoleIDs(), "5") OR ListContains(session.User.getRoleIDs(), "6")>
			...
			<cfoutput>
				<script language="JavaScript">
					var msg = "Intake (#trim(IntakeID)#) has been restored";
