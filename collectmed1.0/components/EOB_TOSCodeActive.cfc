...
<cfcomponent name="eob_toscodeActive" output="false" alias="eob_toscodeActive">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="init" output="false" returntype="eob_toscodeActive">
		...
		<cfif isDefined("arguments.stValues")>
			<cfif StructKeyExists(stValues, "RecordID") AND stValues.RecordID NEQ 0>
				...
				<cfscript>
					variables.Code = qGetEOB_TOSCode.Code;
					variables.Description = qGetEOB_TOSCode.Description;
					variables.Active = qGetEOB_TOSCode.Active;
					variables.InactiveCode = qGetEOB_TOSCode.InactiveCode;
					variables.DateModified = qGetEOB_TOSCode.DateModified;
				</cfscript>
			</cfif>
	...
	<cffunction name="create" output="false" access="public">
		...
		<cfargument name="obj" required="true" type="EOB_TOSCodeActive" />
		...
		<cfscript>
			read(qCreateEOB_TOSCode.RecordID);
			arguments.obj.setRecordID(qCreateEOB_TOSCode.RecordID);
		</cfscript>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="read" output="false" access="public" returntype="EOB_TOSCodeActive">
				
		<cfargument name="RecordID" required="yes" type="numeric" />		
		<cfset var qGetEOB_TOSCode = "" />		
		
		<cfset qGetEOB_TOSCode = getEOB_TOSCode(trim(RecordID)) />
		...
		<!-------------------------------------------------------------------------------------->			
		<cfscript>
			this.setRecordID(qGetEOB_TOSCode.RecordID);
			this.setCode(qGetEOB_TOSCode.Code);
			this.setDescription(qGetEOB_TOSCode.Description);
			this.setActive(qGetEOB_TOSCode.Active);
			this.setInactiveCode(qGetEOB_TOSCode.InactiveCode);
			this.setDateCreated(qGetEOB_TOSCode.DateCreated);
			this.setDateModified(qGetEOB_TOSCode.DateModified);				
			return this;
		</cfscript>
