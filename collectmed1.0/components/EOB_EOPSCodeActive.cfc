...
<cfcomponent name="eob_eopscodeActive" output="false" alias="eob_eopscodeActive">
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="init" output="false" returntype="eob_eopscodeActive">
		...
		<cfif isDefined("arguments.stValues")>
			<cfif StructKeyExists(stValues, "RecordID") AND stValues.RecordID NEQ 0>
				<cfquery name="qGetEOB_EOPSCode" datasource="#trim(variables.ds)#">
				...
				<cfscript>
					variables.Code = qGetEOB_EOPSCode.Code;
					variables.Description = qGetEOB_EOPSCode.Description;
					variables.Active = qGetEOB_EOPSCode.Active;
					variables.InactiveCode = qGetEOB_EOPSCode.InactiveCode;
					variables.DateModified = qGetEOB_EOPSCode.DateModified;
				</cfscript>
			</cfif>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="getEOB_EOPSCode" output="false" access="public" returntype="query">
	...
	<cffunction name="create" output="false" access="public">
		...
		<cfargument name="obj" required="true" type="EOB_EOPSCodeActive" />
		
		<cfset var qCreateEOB_EOPSCode = "" />		
		...
		<cftransaction isolation="read_committed">
			...
			<cfquery name="qCreateEOB_EOPSCode" datasource="#trim(variables.ds)#">
				INSERT INTO eob_eopscode (Code,Description,InactiveCode)
				VALUES (						
		...
		<cfscript>
			read(qCreateEOB_EOPSCode.RecordID);
			arguments.obj.setRecordID(qCreateEOB_EOPSCode.RecordID);
		</cfscript>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="read" output="false" access="public" returntype="EOB_EOPSCodeActive">
				
		<cfargument name="RecordID" required="yes" type="numeric" />		
		<cfset var qGetEOB_EOPSCode = "" />		
		
		<cfset qGetEOB_EOPSCode = getEOB_EOPSCode(trim(RecordID)) />
		...
		<!-------------------------------------------------------------------------------------->			
		<cfscript>
			this.setRecordID(qGetEOB_EOPSCode.RecordID);
			this.setCode(qGetEOB_EOPSCode.Code);
			this.setDescription(qGetEOB_EOPSCode.Description);
			this.setActive(qGetEOB_EOPSCode.Active);
			this.setInactiveCode(qGetEOB_EOPSCode.InactiveCode);
			this.setDateCreated(qGetEOB_EOPSCode.DateCreated);
			this.setDateModified(qGetEOB_EOPSCode.DateModified);				
			return this;
		</cfscript>
	...
	<cffunction name="update" output="false" access="public">
		...
		<cftry>
			...
			<cfquery name="qUpdateEOB_EOPSCode" datasource="#trim(variables.ds)#">
				UPDATE eob_eopscode  SET
	...
	<cffunction name="delete" output="false" access="public" returntype="void">
		...
		<cfargument name="obj" required="true" type="EOB_EOPSCodeActive" />
		<cfset var qDeleteEOB_EOPSCode = "">

		<cfquery name="qDeleteEOB_EOPSCode" datasource="#trim(variables.ds)#" result="status">
			DELETE
			FROM eob_eopscode
