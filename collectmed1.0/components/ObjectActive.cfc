...
<cfcomponent name="objectActive" output="false" alias="objectActive">

	<cfproperty name="ObjectID" type="numeric" default="0" />
	<cfproperty name="ObjectName" type="string" default="" />
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="init" output="false" returntype="objectActive">
		...
		<cfif isDefined("arguments.stValues")>
			<cfif StructKeyExists(stValues, "ObjectID") AND stValues.ObjectID NEQ 0>
				...
				<cfscript>
					variables.ObjectName = qGetObject.ObjectName;
					variables.Description = qGetObject.Description;
					variables.Active = qGetObject.Active;
					variables.InactiveCode = qGetObject.InactiveCode;
					variables.DateModified = qGetObject.DateModified;
				</cfscript>
			</cfif>
			<cfif StructKeyExists(stValues, "ObjectID")><cfset variables.ObjectID = stValues.ObjectID /></cfif>
			<cfif StructKeyExists(stValues, "ObjectName")><cfset variables.ObjectName = stValues.ObjectName /></cfif>
	...
	<!-------------------------------------------------------------------------------------->
	<!--- GET and SET methods for ObjectName.                                            --->
	<!-------------------------------------------------------------------------------------->
	<cffunction name="getObjectName" acess="public" output="false" returntype="any">
		<cfreturn variables.ObjectName>
	</cffunction>
	
	<cffunction name="setObjectName" acess="public" output="false" returntype="boolean">
		
		<cfargument name="ObjectName" required="true" type="any" />
			
		<cfset variables.ObjectName = arguments.ObjectName />					
		<cfset variables.changedVariables = ListAppend(variables.changedVariables, "ObjectName") />		
	...
	<cffunction name="save" output="false" access="remote">
		...
		<cfif this.getObjectID() EQ 0>
			<cfset create(this)>			
		<cfelse>
			<cfset update(this)>
		</cfif>
		
		<cfreturn this.getObjectID()>	
	...
	<cffunction name="create" output="false" access="public">
		...
		<cfargument name="obj" required="true" type="ObjectActive" />
		
		<cfset var qCreateObject = "" />		
		
		<cfset var localObjectName = arguments.obj.getObjectName() />
		...
		<cfscript>
			read(qCreateObject.ObjectID);
			arguments.obj.setObjectID(qCreateObject.ObjectID);
		</cfscript>
	...
	<!-------------------------------------------------------------------------------------->	
	<cffunction name="read" output="false" access="public" returntype="ObjectActive">
				
		<cfargument name="ObjectID" required="yes" type="numeric" />		
		<cfset var qGetObject = "" />		
		
		<cfset qGetObject = getObject(trim(ObjectID)) />
		...
		<!-------------------------------------------------------------------------------------->			
		<cfscript>
			this.setObjectID(qGetObject.ObjectID);
			this.setObjectName(qGetObject.ObjectName);
			this.setDescription(qGetObject.Description);
			this.setActive(qGetObject.Active);
			this.setInactiveCode(qGetObject.InactiveCode);
			this.setDateCreated(qGetObject.DateCreated);
			this.setDateModified(qGetObject.DateModified);				
			return this;
		</cfscript>
	...
	<cffunction name="update" output="false" access="public">
		...
		<cftry>
			...
			<cfquery name="qUpdateObject" datasource="#trim(variables.ds)#">
				...
				WHERE ObjectID = <cfqueryparam value="#trim(arguments.obj.getObjectID())#" cfsqltype="CF_SQL_INTEGER">
			</cfquery>		
